...
describe('grid hook boundary helpers', () => {
  test('resolves live viewport, resize, overlay, and camera helpers', () => {
    ...
    expect(resolveResizeGuideColumn({ activeResizeColumn: null, cursor: 'cell', header: { kind: 'column', index: 4 } })).toBeNull()
    ...
    expect(resolveResizeGuideRow({ activeResizeRow: null, cursor: 'cell', header: { kind: 'row', index: 6 } })).toBeNull()
  ...
  test('keeps workbook render hook behind the runtime camera boundary', () => {
    ...
    expect(geometryRuntimeSource).toContain('gridRuntimeHost.dispose()')
