...
describe('GridSelectionVisualOverlay', () => {
  ...
  test('builds Excel-style continuous DOM visual rects for body range selections', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'selection-fill', bounds: expect.objectContaining({ x: 136, y: 40, width: 300, height: 60 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 236, y: 40, width: 1, height: 60 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 136, y: 60, width: 300, height: 1 }) }),
        expect.objectContaining({ role: 'selection-border', bounds: expect.objectContaining({ x: 136, y: 40, width: 300, height: 60 }) }),
        expect.objectContaining({ role: 'fill-handle', bounds: expect.objectContaining({ x: 432, y: 96, width: 8, height: 8 }) }),
      ]),
    )
  ...
  test('builds visible header and body rects for axis selections without an extra active-cell box', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 136, y: 0, width: 100, height: 20 }) }),
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 236, y: 0, width: 100, height: 20 }) }),
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 336, y: 0, width: 100, height: 20 }) }),
        expect.objectContaining({ role: 'selection-fill', bounds: expect.objectContaining({ x: 136, y: 20, width: 300, height: 220 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 236, y: 20, width: 1, height: 220 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 136, y: 40, width: 300, height: 1 }) }),
      ]),
    )
    expect(rects.filter((rect) => rect.role === 'selection-fill')).toHaveLength(1)
  ...
  test('keeps row-axis selections off the row-header seam by omitting active-cell chrome', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 0, y: 160, width: 36, height: 20 }) }),
        expect.objectContaining({ role: 'header-seam-cover', bounds: expect.objectContaining({ x: 35, y: 160, width: 1, height: 20 }) }),
        expect.objectContaining({ role: 'selection-fill', bounds: expect.objectContaining({ x: 36, y: 160, width: 524, height: 80 }) }),
      ]),
    )
    expect(rects.some((rect) => rect.role === 'active-border')).toBe(false)
    expect(rects).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 1, width: 38 }) })]),
    )
  })
  ...
  test('selected axis headers own full header cells so static gridlines cannot double-paint seams', () => {
    ...
    expect(columnRects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ key: 'header-fill:column:1', bounds: expect.objectContaining({ x: 136, y: 0, width: 100, height: 20 }) }),
        expect.objectContaining({
          key: 'header-seam-cover:column:1',
          bounds: expect.objectContaining({ x: 136, y: 19, width: 100, height: 1 }),
        }),
      ]),
    )
    expect(rowRects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ key: 'header-fill:row:7', bounds: expect.objectContaining({ x: 0, y: 160, width: 36, height: 20 }) }),
        expect.objectContaining({
          key: 'header-seam-cover:row:7',
          bounds: expect.objectContaining({ x: 35, y: 160, width: 1, height: 20 }),
        }),
      ]),
    )
  })
  ...
  test('clips scrolled axis header fills to their panes', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 136, y: 0, width: 50, height: 20 }) }),
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 186, y: 0, width: 100, height: 20 }) }),
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 286, y: 0, width: 100, height: 20 }) }),
      ]),
    )
    expect(rects).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'header-fill', bounds: expect.objectContaining({ x: 97, y: 1, width: 98, height: 20 }) }),
      ]),
    )
  })
  ...
  test('draws a single selected cell with active-cell chrome', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'active-border', bounds: expect.objectContaining({ x: 236, y: 100, width: 100, height: 20 }) }),
        expect.objectContaining({ role: 'fill-handle', bounds: expect.objectContaining({ x: 332, y: 116, width: 8, height: 8 }) }),
      ]),
    )
    expect(rects.some((rect) => rect.role === 'selection-border')).toBe(false)
  ...
  test('keeps bounded range fill visible when visible index caches lag behind range geometry', () => {
    ...
    expect(rects).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ role: 'selection-fill', bounds: expect.objectContaining({ x: 136, y: 40, width: 200, height: 40 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 236, y: 40, width: 1, height: 40 }) }),
        expect.objectContaining({ role: 'selection-gridline', bounds: expect.objectContaining({ x: 136, y: 60, width: 200, height: 1 }) }),
        expect.objectContaining({ role: 'selection-border', bounds: expect.objectContaining({ x: 136, y: 40, width: 200, height: 40 }) }),
      ]),
    )
    expect(rects.filter((rect) => rect.role === 'selection-fill')).toHaveLength(1)
  ...
  test('builds hover chrome in the DOM overlay without covering the selected range', () => {
    ...
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({ role: 'hover-fill', bounds: expect.objectContaining({ x: 437, y: 121, width: 98, height: 18 }) }),
      ]),
    )
  ...
  test('chrome-only mode leaves fills to TypeGPU and paints crisp DOM selection chrome', async () => {
    ...
    expect(headerSeamCover).toBeInstanceOf(HTMLElement)
    ...
    expect(headerFill?.style.backgroundColor).toBe('rgba(12, 130, 88, 0.16)')
    expect(headerSeamCover?.style.opacity).toBe('')
    expect(headerSeamCover?.style.backgroundColor).toBe('rgb(206, 224, 214)')
    ...
    expect(strokeEdges.find((edge) => edge.dataset.gridSelectionVisualEdge === 'left')?.style.top).toBe('0px')
    expect(strokeEdges.find((edge) => edge.dataset.gridSelectionVisualEdge === 'left')?.style.bottom).toBe('1px')
    expect(strokeEdges.find((edge) => edge.dataset.gridSelectionVisualEdge === 'right')?.style.right).toBe('0px')
    expect(strokeEdges.find((edge) => edge.dataset.gridSelectionVisualEdge === 'right')?.style.top).toBe('0px')
    expect(strokeEdges.find((edge) => edge.dataset.gridSelectionVisualEdge === 'right')?.style.bottom).toBe('1px')
