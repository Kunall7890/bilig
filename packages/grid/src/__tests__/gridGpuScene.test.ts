...
describe('gridGpuScene', () => {
  ...
  test('adds continuous GPU selection fill and one outline for body ranges', () => {
    const scene = buildGridGpuScene({
      engine: makeEngine({}),
      columnWidths: {},
      gridMetrics: getGridMetrics(),
    ...
    expect(selectionFills).toHaveLength(1)
    expect(selectionFills).toContainEqual({
      x: 100,
      y: 48,
      width: 200,
      height: 72,
    ...
    expect(scene.fillRects).not.toContainEqual({ x: 101, y: 49, width: 98, height: 22, color: SELECTION_FILL_COLOR })
    expect(scene.borderRects).toContainEqual({
      x: 100,
      y: 48,
      width: 200,
