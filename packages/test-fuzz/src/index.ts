...
function serializeArtifactValue(value: unknown): unknown {
  ...
  if (isRecord(value)) {
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, serializeArtifactValue(entry)]))
  }
  return value
}
...
export function promoteCapturedArtifact(options: PromoteCapturedArtifactOptions): string {
  ...
  if (!isRecord(artifact) || typeof artifact['suite'] !== 'string' || typeof artifact['seed'] !== 'number') {
...
export function captureCounterexample<Ts extends unknown[]>(options: CaptureCounterexampleOptions<Ts>): string {
