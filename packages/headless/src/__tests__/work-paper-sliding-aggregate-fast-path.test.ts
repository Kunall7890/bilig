...
describe('WorkPaper sliding aggregate fast path', () => {
  it('evaluates far shifted aggregate windows from column page summaries', () => {
    const rowCount = 4_224
    const workbook = WorkPaper.buildFromSheets({
      Bench: Array.from({ length: rowCount }, (_, row) => {
        if (row === 0) {
          return [null, '=SUM(A4097:A4224)', '=COUNT(A4097:A4224)', '=AVERAGE(A4097:A4224)', '=MIN(A4097:A4224)', '=MAX(A4097:A4224)']
        }
        if (row >= 4_096) {
          return [row - 4_095]
        }
    ...
    expect(workbook.getCellValue(cell(sheetId, 0, 4))).toEqual({ tag: ValueTag.Number, value: 1 })
    expect(workbook.getCellValue(cell(sheetId, 0, 5))).toEqual({ tag: ValueTag.Number, value: 128 })
    const counters = workbook.getPerformanceCounters()
    expect(counters.formulasBound).toBe(0)
    expect(counters.directAggregatePageEvaluations).toBe(5)
    expect(counters.directAggregatePageFullHits).toBe(5)
