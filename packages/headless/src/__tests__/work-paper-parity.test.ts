...
describe('WorkPaper parity surface', () => {
  it('covers static factories, config inventory, and registry APIs', () => {
    ...
    expect(() => WorkPaper.getLanguage(TEST_LANGUAGE_CODE)).toThrow(WorkPaperLanguageNotRegisteredError)
    ...
    expect(() => WorkPaper.registerLanguage(TEST_LANGUAGE_CODE, { functions: {} })).toThrow(WorkPaperLanguageAlreadyRegisteredError)
    ...
    expect(WorkPaper.getAllFunctionPlugins().map((plugin) => plugin.id)).toContain(CUSTOM_PLUGIN.id)
  })
  ...
  it('covers all config keys through build, clone, and update paths', () => {
    ...
    expect(snapshot.stringifyDateTime?.({ year: 2024, month: 1, day: 2, hours: 3, minutes: 4, seconds: 5 }, 'YYYY-MM-DD HH:mm:ss')).toBe(
      'date',
    )
    expect(snapshot.stringifyDuration?.({ hours: 3, minutes: 4, seconds: 5 }, 'HH:mm:ss')).toBe('duration')
    ...
    if (snapshot.context && typeof snapshot.context === 'object' && !Array.isArray(snapshot.context)) {
  ...
  it('covers the read surface, formula helpers, and dependency helpers', () => {
    ...
    expect(workbook.getCellFormula(cell(dataId, 0, 1))).toBe('=HYPERLINK("https://example.com","Docs")')
    ...
    expect(workbook.getAllSheetsSerialized().Data?.[0]?.[1]).toBe('=HYPERLINK("https://example.com","Docs")')
  ...
  it('covers mutations, preflights, history controls, clipboard, and fill helpers', () => {
    ...
    expect(workbook.isItPossibleToMoveCells({ start: cell(sheetId, 0, 0), end: cell(sheetId, 0, 0) }, cell(sheetId, 1, 0))).toBe(true)

    workbook.setCellContents(cell(sheetId, 1, 0), 10)
    workbook.setCellContents(cell(sheetId, 1, 1), 20)
    workbook.moveCells({ start: cell(sheetId, 1, 0), end: cell(sheetId, 1, 1) }, cell(sheetId, 2, 0))
  ...
  it('covers sheet lifecycle, named expressions, events, and internal adapters', () => {
    ...
    expect(workbook.getAllNamedExpressionsSerialized()).toEqual([{ name: 'Rate', expression: '=1', scope: dataId, options: undefined }])
    ...
    expect(() => workbook.addNamedExpression('Rate', '=3', dataId)).toThrow(WorkPaperNamedExpressionNameIsAlreadyTakenError)
    expect(() => workbook.changeNamedExpression('Missing', '=1', dataId)).toThrow(WorkPaperNamedExpressionDoesNotExistError)
    expect(() => workbook.addNamedExpression('RelativeName', '=A1', dataId)).toThrow(WorkPaperNoRelativeAddressesAllowedError)
    ...
    expect(workbook.columnSearch.find(dataId, 0, (value) => value.tag === ValueTag.Number)).toEqual([cell(dataId, 0, 0)])
