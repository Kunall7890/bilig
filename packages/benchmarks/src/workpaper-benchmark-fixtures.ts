...
export function buildMixedContentSheet(rowCount: number): WorkPaperSheet {
  return Array.from({ length: rowCount }, (_, index) => {
    ...
    return [rowNumber, rowNumber * 10, `label-${rowNumber}`, rowNumber % 2 === 0, `=A${rowNumber}+B${rowNumber}`, `=E${rowNumber}*2`]
  })
}

export function buildMultiSheetLiteralSheets(sheetCount: number, rows: number, cols: number): Record<string, WorkPaperSheet> {
  return Object.fromEntries(
    Array.from({ length: sheetCount }, (_, sheetIndex) => [
      `Sheet${sheetIndex + 1}`,
      Array.from({ length: rows }, (_rowValue, rowIndex) =>
        Array.from({ length: cols }, (_colValue, colIndex) => sheetIndex * rows * cols + rowIndex * cols + colIndex + 1),
      ),
    ]),
  )
}
...
export function buildLookupSheet(rowCount: number): WorkPaperSheet {
  const rows: Array<Array<number | string>> = [['Key', 'Value', '', Math.floor(rowCount / 2), '=MATCH(D1,A2:A20001,0)']]
...
export function buildApproxLookupSheet(rowCount: number): WorkPaperSheet {
  const rows: Array<Array<number | string>> = [['Key', 'Value', '', Math.floor(rowCount / 2) + 0.5, '']]
  for (let index = 1; index <= rowCount; index += 1) {
    rows.push([index, index * 10])
  }
  const headerRow = rows[0]
...
export function buildDynamicArraySheet(rowCount: number): WorkPaperSheet {
  const rows: Array<Array<number | string>> = [[0, Math.floor(rowCount / 2), `=FILTER(A2:A${rowCount + 1},A2:A${rowCount + 1}>B1)`]]
  for (let index = 1; index <= rowCount; index += 1) {
    rows.push([index])
  }
  return rows
...
export function buildConditionalAggregationSheet(rowCount: number, formulaCopies: number): WorkPaperSheet {
  const rows: Array<Array<number | string>> = [
    [
      ...
      '',
      'A',
      ...Array.from({ length: formulaCopies }, () => `=SUMIF(A2:A${rowCount + 1},D1,B2:B${rowCount + 1})`),
...
export function buildConditionalAggregationSharedCriteriaSheet(rowCount: number, criteriaCount: number): WorkPaperSheet {
  const criteriaCells = Array.from({ length: criteriaCount }, (_, index) => String.fromCharCode(65 + (index % 4)))
  const rows: Array<Array<number | string>> = [
    [
      'Group',
      'Value',
      '',
      ...criteriaCells,
      ...criteriaCells.map((_, index) => `=SUMIF(A2:A${rowCount + 1},${columnLabel(3 + index)}1,B2:B${rowCount + 1})`),
    ],
  ]
  for (let index = 1; index <= rowCount; index += 1) {
...
export function buildConditionalAggregationMixedSheet(rowCount: number, formulaCopies: number): WorkPaperSheet {
  const rows: Array<Array<number | string>> = [
    [
      'Group',
      'Amount',
...
export function range(sheet: number, startRow: number, startCol: number, endRow: number, endCol: number): WorkPaperCellRange {
  return {
