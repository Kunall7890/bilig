...
describe('@bilig/workbook action input api', () => {
  ...
  it('rejects unknown described object input fields before model code runs', () => {
    const model = defineModel({
      name: 'closed-action-input-model',
    ...
    expect(
      planWorkbookAction(model, 'write', {
        value: 7,
        options: {
          label: 'seed',
          typo: true,
      ...
      status: 'failed',
      modelName: 'closed-action-input-model',
      actionName: 'write',
      input: {
        options: {
          label: 'seed',
      ...
      errors: [
        {
          code: 'invalid_action_input',
          message: 'Action input at input.options.typo is not supported',
        },
      ],
    })
    ...
    expect(
      ...
      errors: [
        {
          code: 'invalid_action_input',
          message: 'Action input at input.valueTypo is not supported',
        },
      ],
    })
  })

  it('keeps unconstrained object action input generic', () => {
    const model = defineModel({
      name: 'open-object-action-input-model',
    ...
    expect(planWorkbookAction(model, 'write', { custom: { ownedBy: 'consumer' } })).toEqual({
      status: 'planned',
      plan: expect.objectContaining({
        modelName: 'open-object-action-input-model',
        actionName: 'write',
        input: {
          custom: {
            ownedBy: 'consumer',
