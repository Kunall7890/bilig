...
function decodeStringValue(rawValue: number, pooledStrings: readonly string[], outputStrings: readonly string[]): string {
...
function readScalarValue(kernel: KernelInstance, cellIndex: number, pooledStrings: readonly string[]): CellValue {
  ...
  if (tag === ValueTag.String) {
    return {
      tag,
      value: decodeStringValue(kernel.readStringIds()[cellIndex] ?? 0, pooledStrings, kernel.readOutputStrings()),
      stringId: 0,
    }
  }
...
describe('wasm kernel array/info dispatch', () => {
  it('keeps array, text, and info builtin dispatch stable across refactors', async () => {
    ...
    const packed = packPrograms([
      ...
      [encodePushNumber(0), encodePushNumber(1), encodePushNumber(2), encodePushNumber(3), encodeCall(BuiltinId.Choose, 4), encodeRet()],
      [encodePushString(3), encodePushBoolean(true), encodePushRange(3), encodeCall(BuiltinId.Textjoin, 3), encodeRet()],
      [encodeCall(BuiltinId.Na, 0), encodeRet()],
      [encodePushError(ErrorCode.Ref), encodePushString(0), encodeCall(BuiltinId.Iferror, 2), encodeRet()],
      [encodeCall(BuiltinId.Na, 0), encodePushString(1), encodeCall(BuiltinId.Ifna, 2), encodeRet()],
      [encodePushString(2), encodeCall(BuiltinId.T, 1), encodeRet()],
      [encodePushNumber(0), encodeCall(BuiltinId.T, 1), encodeRet()],
      [encodePushBoolean(false), encodeCall(BuiltinId.T, 1), encodeRet()],
    ...
    expect(readScalarValue(kernel, ownerBase + 17, pooledStrings)).toEqual({
      tag: ValueTag.Number,
      value: 1,
    })
