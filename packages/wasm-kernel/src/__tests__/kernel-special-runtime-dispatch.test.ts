...
describe('wasm kernel special runtime dispatch', () => {
  ...
  it('keeps SUMPRODUCT on the current scalar output path', async () => {
    ...
    kernel.uploadPrograms(
      new Uint32Array([encodePushRange(0), encodePushRange(1), encodeCall(BuiltinId.Sumproduct, 2), encodeRet()]),
      new Uint32Array([0]),
      new Uint32Array([4]),
  ...
  it('keeps IRR, MIRR, XNPV, and XIRR on the current finance root path', async () => {
    ...
    const packed = packPrograms([
      ...
      [encodePushRange(1), encodePushNumber(0), encodePushNumber(1), encodeCall(BuiltinId.Mirr, 3), encodeRet()],
      [encodePushNumber(0), encodePushRange(2), encodePushRange(3), encodeCall(BuiltinId.Xnpv, 3), encodeRet()],
