export function erfApprox(value: f64): f64 {
  ...
  const y =
    1.0 - ((((1.061405429 * t - 1.453152027) * t + 1.421413741) * t - 0.284496736) * t + 0.254829592) * t * Math.exp(-absolute * absolute)
  return sign * y
}
...
export function inverseStandardNormal(value: f64): f64 {
  ...
  if (value < lower) {
    ...
    return (
      (((((-0.007784894002430293 * q - 0.3223964580411365) * q - 2.400758277161838) * q - 2.549732539343734) * q + 4.374664141464968) * q +
        2.938163982698783) /
  ...
  const r = q * q
  return (
    ((((((-39.69683028665376 * r + 220.9460984245205) * r - 275.9285104469687) * r + 138.357751867269) * r - 30.66479806614716) * r +
      2.506628277459239) *
      q) /
...
export function besselKValue(x: f64, order: i32): f64 {
  const shiftedOrder = <f64>order + BESSEL_EPSILON
  return (
    ((Math.PI / 2.0) * (besselSeries(-shiftedOrder, x, false) - besselSeries(shiftedOrder, x, false))) / Math.sin(Math.PI * shiftedOrder)
  )
}
...
function betaContinuedFraction(x: f64, alpha: f64, beta: f64): f64 {
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration += 1) {
    ...
    factor = (-(alpha + <f64>iteration) * (qab + <f64>iteration) * x) / ((alpha + step) * (qap + step))
    d = 1.0 + factor * d
    if (Math.abs(d) < tiny) {
      d = tiny
    }
...
function regularizedBeta(x: f64, alpha: f64, beta: f64): f64 {
  if (!isFinite(x) || !isFinite(alpha) || !isFinite(beta) || alpha <= 0.0 || beta <= 0.0 || x < 0.0 || x > 1.0) {
    return NaN
  }
  if (x == 0.0) {
    return 0.0
...
export function betaDistributionDensity(x: f64, alpha: f64, beta: f64, lowerBound: f64 = 0.0, upperBound: f64 = 1.0): f64 {
  if (
    !isFinite(x) ||
    !isFinite(alpha) ||
    !isFinite(beta) ||
  ...
  return Math.exp((alpha - 1.0) * Math.log(normalized) + (beta - 1.0) * Math.log(1.0 - normalized) - logBeta(alpha, beta) - Math.log(scale))
}

export function betaDistributionCdf(x: f64, alpha: f64, beta: f64, lowerBound: f64 = 0.0, upperBound: f64 = 1.0): f64 {
  if (
    !isFinite(x) ||
    !isFinite(alpha) ||
    !isFinite(beta) ||
...
export function betaDistributionInverse(probability: f64, alpha: f64, beta: f64, lowerBound: f64 = 0.0, upperBound: f64 = 1.0): f64 {
  if (
    !isFinite(probability) ||
    !isFinite(alpha) ||
    !isFinite(beta) ||
...
export function inverseFDistribution(probability: f64, degreesFreedom1: f64, degreesFreedom2: f64): f64 {
  if (
    !isFinite(probability) ||
    !isFinite(degreesFreedom1) ||
    !isFinite(degreesFreedom2) ||
  ...
  const transformed = inverseRegularizedBeta(probability, degreesFreedom1 / 2.0, degreesFreedom2 / 2.0)
...
export function inverseStudentT(probability: f64, degreesFreedom: f64): f64 {
  if (!isFinite(probability) || !isFinite(degreesFreedom) || !(probability > 0.0 && probability < 1.0) || degreesFreedom < 1.0) {
    return NaN
  }
  if (probability == 0.5) {
    return 0.0
...
function logCombination(total: i32, chosen: i32): f64 {
  ...
  return logGamma(<f64>total + 1.0) - logGamma(<f64>chosen + 1.0) - logGamma(<f64>(total - chosen) + 1.0)
}
...
export function binomialProbability(successes: i32, trials: i32, probability: f64): f64 {
  ...
  return Math.exp(
    logCombination(trials, successes) + <f64>successes * Math.log(probability) + <f64>(trials - successes) * Math.log(1.0 - probability),
  )
}
...
export function hypergeometricProbability(sampleSuccesses: i32, sampleSize: i32, populationSuccesses: i32, populationSize: i32): f64 {
  if (
    sampleSuccesses < 0 ||
    sampleSize < 0 ||
...
export function gammaDistributionDensity(x: f64, alpha: f64, beta: f64): f64 {
  if (!isFinite(x) || !isFinite(alpha) || !isFinite(beta) || x < 0.0 || alpha <= 0.0 || beta <= 0.0) {
    return NaN
  }
  if (x == 0.0) {
    if (alpha == 1.0) {
  ...
  return Math.exp((alpha - 1.0) * Math.log(x) - x / beta - logGamma(alpha) - alpha * Math.log(beta))
}
...
export function inverseChiSquare(probability: f64, degreesFreedom: f64): f64 {
  ...
  const approximationFactor = isNaN(z) ? NaN : 1.0 - 2.0 / (9.0 * degreesFreedom) + z * Math.sqrt(2.0 / (9.0 * degreesFreedom))
  let estimate =
    isFinite(approximationFactor) && approximationFactor > 0.0 ? degreesFreedom * Math.pow(approximationFactor, 3.0) : degreesFreedom
  if (!(estimate > 0.0) || !isFinite(estimate)) {
