...
export function textLength(tag: u8, value: f64, stringLengths: Uint32Array, outputStringLengths: Uint32Array): i32 {
  ...
  while (index < bytes.length) {
    ...
    if (b0 >= 0xe0 && b0 <= 0xef) {
      if (index + 2 < bytes.length) {
        ...
        const validSecond = (b1 & 0xc0) == 0x80 && (b2 & 0xc0) == 0x80 && (b0 != 0xe0 || b1 >= 0xa0) && (b0 != 0xed || b1 < 0xa0)
        if (validSecond) {
    ...
    if (b0 >= 0xf0 && b0 <= 0xf4) {
      if (index + 3 < bytes.length) {
        ...
        const validFour =
          (b1 & 0xc0) == 0x80 && (b2 & 0xc0) == 0x80 && (b3 & 0xc0) == 0x80 && (b0 != 0xf0 || b1 >= 0x90) && (b0 != 0xf4 || b1 < 0x90)
        if (validFour) {
          let codePoint = ((b0 & 0x07) << 18) | ((b1 & 0x3f) << 12) | ((b2 & 0x3f) << 6) | (b3 & 0x3f)
          codePoint -= 0x10000
          result += String.fromCharCode(<i32>(0xd800 | (codePoint >> 10)), <i32>(0xdc00 | (codePoint & 0x03ff)))
          index += 4
          continue
        }
      }
...
export function replaceBytesText(text: string, start: i32, count: i32, replacement: string): string {
  ...
  const resultBytes = new Uint8Array(zeroBasedStart + replacementBytes.length + (textBytes.length - zeroBasedEnd))
  let cursor = 0
...
export function poolString(stringId: i32, stringOffsets: Uint32Array, stringLengths: Uint32Array, stringData: Uint16Array): string | null {
  if (stringId < 0 || stringId >= stringLengths.length) {
    return null
  }
