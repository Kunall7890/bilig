...
import { fDistributionCdf, regularizedUpperGamma, standardNormalCdf, studentTCdf } from './distributions'
import {
  inputCellNumeric,
...
export function chiSquareTestPValue(
  ...
  const degrees = actualRows > 1 && actualCols > 1 ? (actualRows - 1) * (actualCols - 1) : actualRows > 1 ? actualRows - 1 : actualCols - 1
...
export function fTestPValue(
  ...
  const upperTail = 1.0 - fDistributionCdf(numeratorVariance / denominatorVariance, numeratorDegrees, denominatorDegrees)
  ...
  if (testType == 1) {
    ...
    if (testType == 2) {
      const pooledVariance =
        (<f64>(first.length - 1) * firstVariance + <f64>(second.length - 1) * secondVariance) / <f64>(first.length + second.length - 2)
      if (!(pooledVariance > 0.0)) {
        return -(<f64>ErrorCode.Div0)
      }
      statistic = (firstMean - secondMean) / Math.sqrt(pooledVariance * (1.0 / <f64>first.length + 1.0 / <f64>second.length))
      ...
      const welchDenominator = (firstTerm * firstTerm) / <f64>(first.length - 1) + (secondTerm * secondTerm) / <f64>(second.length - 1)
