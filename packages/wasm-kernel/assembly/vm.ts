...
function writeCellValue(cellIndex: i32, tag: u8, value: f64): void {
  ...
  if (tag == ValueTag.String) {
    stringIds[cellIndex] = value >= OUTPUT_STRING_BASE ? 0x80000000 | <u32>(value - OUTPUT_STRING_BASE) : <u32>value
  } else {
...
function computeBinaryScalarResult(opcode: i32, leftTag: u8, leftValue: f64, rightTag: u8, rightValue: f64): void {
...
export function uploadPrograms(programs: Uint32Array, offsets: Uint32Array, lengths: Uint32Array, targets: Uint32Array): void {
...
export function writeCells(nextTags: Uint8Array, nextNumbers: Float64Array, nextStringIds: Uint32Array, nextErrors: Uint16Array): void {
...
export function readCachedRangeSumTag(rangeIndex: u32): u8 {
  return rangeIndex < <u32>sumRangeCacheTags.length ? sumRangeCacheTags[rangeIndex] : UNCACHED_AGGREGATE_TAG
}
...
function evalProgram(cellIndex: i32, formulaIndex: i32): void {
  ...
  for (let pc = start; pc < end; pc++) {
    ...
    if (
      ...
      if (rightKind == STACK_KIND_RANGE || leftKind == STACK_KIND_RANGE || rightKind == STACK_KIND_ARRAY || leftKind == STACK_KIND_ARRAY) {
        const leftRows = slotRows(sp - 2)
        const leftCols = slotCols(sp - 2)
        const rightRows = slotRows(sp - 1)
        const rightCols = slotCols(sp - 1)
        if (leftRows == i32.MIN_VALUE || leftCols == i32.MIN_VALUE || rightRows == i32.MIN_VALUE || rightCols == i32.MIN_VALUE) {
          writeScalar(sp - 2, <u8>ValueTag.Error, ErrorCode.Ref)
          sp--
          continue
        }
        const rows = leftRows == rightRows ? leftRows : leftRows == 1 ? rightRows : rightRows == 1 ? leftRows : 0
        const cols = leftCols == rightCols ? leftCols : leftCols == 1 ? rightCols : rightCols == 1 ? leftCols : 0
        if (rows == 0 || cols == 0) {
