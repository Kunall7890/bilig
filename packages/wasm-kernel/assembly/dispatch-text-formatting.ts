...
import { STACK_KIND_RANGE, STACK_KIND_SCALAR, copySlotResult, writeArrayResult, writeResult, writeStringResult } from './result-io'
import { allocateOutputString, allocateSpillArrayResult, encodeOutputStringId, writeOutputStringData, writeSpillArrayValue } from './vm'
...
export function tryApplyTextFormattingBuiltin(
    ...
    for (let rowIndex = 0; rowIndex < rows; rowIndex += 1) {
      ...
      for (let colIndex = 0; colIndex < cols; colIndex += 1) {
        if (colIndex < row.length) {
          ...
          writeSpillArrayValue(arrayIndex, outputOffset, <u8>ValueTag.String, encodeOutputStringId(outputStringId))
        } else {
  ...
  if ((builtinId == BuiltinId.Textbefore || builtinId == BuiltinId.Textafter) && argc >= 2 && argc <= 6) {
    if (!scalarArgsOnly(base, argc, kindStack)) {
      return writeResult(base, STACK_KIND_SCALAR, <u8>ValueTag.Error, ErrorCode.Value, rangeIndexStack, valueStack, tagStack, kindStack)
    }
    ...
    return writeStringResult(
      base,
      builtinId == BuiltinId.Textbefore ? text.slice(0, found) : text.slice(found + delimiter.length),
      rangeIndexStack,
      valueStack,
      tagStack,
      kindStack,
  ...
  if (builtinId == BuiltinId.Char && argc == 1) {
    ...
    return writeStringResult(base, String.fromCharCode(integerCode), rangeIndexStack, valueStack, tagStack, kindStack)
  }
  ...
  if (builtinId == BuiltinId.Clean && argc == 1) {
    ...
    return writeStringResult(base, stripControlCharacters(text), rangeIndexStack, valueStack, tagStack, kindStack)
  }

  if ((builtinId == BuiltinId.Asc || builtinId == BuiltinId.Jis || builtinId == BuiltinId.Dbcs) && argc == 1) {
    if (tagStack[base] == ValueTag.Error) {
      return writeResult(base, STACK_KIND_SCALAR, <u8>ValueTag.Error, valueStack[base], rangeIndexStack, valueStack, tagStack, kindStack)
    }
    const text = scalarText(
    ...
    const converted = builtinId == BuiltinId.Asc ? toJapaneseHalfWidth(text) : toJapaneseFullWidth(text)
  ...
  if (builtinId == BuiltinId.Valuetotext && (argc == 1 || argc == 2)) {
    if (argc == 2 && kindStack[base + 1] == STACK_KIND_SCALAR && tagStack[base + 1] == ValueTag.Error) {
      return writeResult(
        base,
        STACK_KIND_SCALAR,
        <u8>ValueTag.Error,
    ...
    if (kindStack[base] == STACK_KIND_SCALAR && tagStack[base] == ValueTag.Error) {
      return writeStringResult(base, errorLabel(<i32>valueStack[base]), rangeIndexStack, valueStack, tagStack, kindStack)
    }
  ...
  if (builtinId == BuiltinId.Text && argc == 2) {
    ...
    if (tagStack[base] == ValueTag.String) {
      ...
      for (let index = 0; index < cleaned.length; index += 1) {
        if (cleaned.charCodeAt(index) == 48 || cleaned.charCodeAt(index) == 35 || cleaned.charCodeAt(index) == 63) {
          hasPlaceholder = true
          break
        }
      }
      if (cleaned.indexOf('@') >= 0 || (!hasPlaceholder && !containsDateTimeTokens(cleaned))) {
        ...
        return writeStringResult(base, formatTextSectionText(textValue, section), rangeIndexStack, valueStack, tagStack, kindStack)
      }
      return writeResult(base, STACK_KIND_SCALAR, <u8>ValueTag.Error, ErrorCode.Value, rangeIndexStack, valueStack, tagStack, kindStack)
    }
