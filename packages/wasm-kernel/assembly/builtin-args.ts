...
export function scalarErrorAt(base: i32, argc: i32, kindStack: Uint8Array, tagStack: Uint8Array, valueStack: Float64Array): f64 {
...
export function collectScalarStatValues(
  ...
  for (let index = 0; index < argc; index++) {
    const numeric = statScalarValue(tagStack[base + index], valueStack[base + index], includeStringsAsZero)
    if (!isNaN(numeric)) {
      values.push(numeric)
    }
  }
