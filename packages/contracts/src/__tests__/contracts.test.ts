    ...
    const decoded = decodeUnknownSync(WorkbookAgentWorkflowRunSchema, {
      ...
      steps: [
        ...
        },
        {
          ...
          status: 'completed',
          summary: 'Prepared 2 semantic formatting commands to highlight numeric outliers on Revenue.',
          updatedAtUnixMs: 115,
        },
      ],
      artifact: {
  ...
  return {
    ...
    rendered: {
      ...
      visibleSceneProof: {
        ...
        currentSceneEpochSignature: `epoch-${String(input.capturedRevision)}`,
        currentSceneOwnershipSignature: visibleSceneOwnershipSignature,
        presentedSceneEpochSignature: `epoch-${String(input.capturedRevision)}`,
        presentedSceneOwnershipSignature: visibleSceneOwnershipSignature,
        currentSceneEpoch: `tile-${String(input.capturedRevision)}`,
        presentedSceneEpoch: `tile-${String(input.capturedRevision)}`,
        currentFillHandleRevision: `fill-${String(input.capturedRevision)}`,
        presentedFillHandleRevision: `fill-${String(input.capturedRevision)}`,
        currentSelectionRevision: `selection-${String(input.capturedRevision)}`,
        presentedSelectionRevision: `selection-${String(input.capturedRevision)}`,
        currentViewportRevision: `viewport-${String(input.capturedRevision)}`,
        presentedViewportRevision: `viewport-${String(input.capturedRevision)}`,
        currentSemanticMutationRevision: String(input.capturedRevision),
        presentedSemanticMutationRevision: String(input.capturedRevision),
        currentWorkbookRevision: String(input.capturedRevision),
        presentedWorkbookRevision: String(input.capturedRevision),
