...
function definedName(name: string, value: ExcelFixtureDefinedName['value'], note?: string): ExcelFixtureDefinedName {
...
function multipleOperations(config: ExcelFixtureMultipleOperationsMock): ExcelFixtureMultipleOperationsMock {
  return config
}
...
export const canonicalWorkbookSemanticsFixtures: readonly ExcelFixtureCase[] = [
  fixture(
    ...
    [output('A2', numberExpected(8.5))],
    {
      ...
      notes: 'Workbook-level names follow the same case-insensitive lookup contract as the engine metadata store.',
    },
  ),
  fixture(
    'names',
    ...
    [output('A1', numberExpected(0.1))],
    {
      ...
      notes: 'Captures the common workbook metadata case where more than one scalar name is bound at once.',
    },
  ),
  fixture(
    'names',
  ...
  fixture(
    ...
    {
      notes: 'Exercises qualified cell references without introducing dynamic-array or table metadata dependencies.',
    },
  ),
  fixture(
    'arithmetic',
  ...
  fixture(
    ...
    {
      notes: 'The target worksheet is intentionally absent so the harness can capture unresolved-sheet error behavior.',
    },
  ),
  fixture(
    'aggregation',
  ...
  fixture(
    'lookup-reference',
    'xlookup-advanced-return-modes',
    'XLOOKUP spills multi-column rows with next-smaller match mode',
    ...
    [output('B1', numberExpected(201)), output('C1', numberExpected(202)), output('D1', numberExpected(203))],
    {
      notes:
        'The lookup value falls between 20 and 30, so match_mode -1 returns the next-smaller row and spills the three-column return array.',
    },
  ),
  {
