...
const CONTEXTUAL_BUILTINS = new Set([
  'CELL',
  'COLUMN',
  'FORMULATEXT',
  'ISFORMULA',
...
export function bindFormula(ast: FormulaNode): BoundFormula {
  const dependencyMetadata = collectFormulaDependencyMetadata(ast)
  ...
  return {
    ast,
    deps: dependencyMetadata.deps,
    symbolicNames: dependencyMetadata.symbolicNames,
    symbolicTables: dependencyMetadata.symbolicTables,
    symbolicSpills: dependencyMetadata.symbolicSpills,
    mode: ast.kind === 'RangeRef' || (!isWasmSafe(ast) && !isTopLevelWasmSafe(ast)) ? FormulaMode.JsOnly : FormulaMode.WasmFastPath,
  }
}
...
export function encodeBuiltin(name: string): BuiltinId {
  const builtins: Record<string, BuiltinId> = {
    ...
    ARRAYFORMULA: BuiltinId.Arrayformula,
