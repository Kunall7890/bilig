...
import { formatStructuredReferenceSpecifier } from './structured-reference-syntax.js'
...
export function serializeFormula(node: FormulaNode, parentPrecedence = 0, parentAssociativity: 'left' | 'right' | null = null): string {
  switch (node.kind) {
    ...
    case 'StructuredRef':
      return `${node.tableName}[${formatStructuredReferenceSpecifier(node)}]`
    case 'CellRef':
