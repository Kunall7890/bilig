...
function lowerExactVectorLookup(node: Extract<FormulaNode, { kind: 'CallExpr' }>, plan: JsPlanInstruction[]): boolean {
...
function lowerApproximateVectorLookup(node: Extract<FormulaNode, { kind: 'CallExpr' }>, plan: JsPlanInstruction[]): boolean {
  const callee = node.callee.toUpperCase()
  const lookupRange = node.args[1]
  if (callee === 'MATCH' && node.args.length === 3 && lookupRange !== undefined && isVectorCellRange(lookupRange)) {
...
function lowerNode(node: FormulaNode, plan: JsPlanInstruction[]): void {
  switch (node.kind) {
    ...
    case 'CellRef':
      plan.push(
        node.sheetName ? { opcode: 'push-cell', sheetName: node.sheetName, address: node.ref } : { opcode: 'push-cell', address: node.ref },
      )
      return
    case 'RangeRef':
      plan.push({
