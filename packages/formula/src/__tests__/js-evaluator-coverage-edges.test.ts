...
import { evaluatePlan, evaluatePlanResult, lowerToPlan, parseFormula, type JsPlanInstruction } from '../index.js'
...
describe('js evaluator coverage edges', () => {
  it('covers stack guard rails for missing, omitted, lambda, range, and top-level lambda results', () => {
    ...
    expect(evaluatePlan([{ opcode: 'unary', operator: '+' }, { opcode: 'return' }], context)).toEqual({
      tag: ValueTag.Error,
    ...
    expect(
      evaluatePlan(
        [{ opcode: 'push-lambda', params: ['x'], body: [{ opcode: 'return' }] }, { opcode: 'unary', operator: '+' }, { opcode: 'return' }],
        context,
      ),
    ...
    expect(
      evaluatePlan(
        [{ opcode: 'push-range', start: 'A1', end: 'B2', refKind: 'cells' }, { opcode: 'unary', operator: '+' }, { opcode: 'return' }],
        context,
      ),
    ).toEqual(num(2))
    expect(
    ...
    expect(
      evaluatePlanResult([{ opcode: 'push-lambda', params: ['x'], body: [{ opcode: 'return' }] }, { opcode: 'return' }], context),
    ).toEqual({ tag: ValueTag.Error, code: ErrorCode.Value })
  ...
  it('covers GROUPBY aggregate fallbacks for empty subsets, missing builtins, and lambda totals', () => {
    const sumResult = evaluatePlanResult(lowerToPlan(parseFormula('GROUPBY(A1:A1,B1:B1,SUM,3,1)')), {
      ...context,
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === 'A1' && end === 'A1') {
          return [text('Group')]
    ...
    const averageResult = evaluatePlanResult(lowerToPlan(parseFormula('GROUPBY(A1:A1,B1:B1,AVERAGE,3,1)')), {
      ...context,
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === 'A1' && end === 'A1') {
          return [text('Group')]
    ...
    const defaultResult = evaluatePlanResult(lowerToPlan(parseFormula('GROUPBY(A1:A1,B1:B1,MIN,3,1)')), {
      ...context,
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === 'A1' && end === 'A1') {
          return [text('Group')]
    ...
    const missingBuiltinResult = evaluatePlanResult(lowerToPlan(parseFormula('GROUPBY(A1:A2,B1:B2,NOPE,3,1)')), {
      ...context,
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === 'A1' && end === 'A2') {
    ...
    const lambdaTotalResult = evaluatePlanResult(lowerToPlan(parseFormula('GROUPBY(A1:A3,B1:B3,LAMBDA(sub,total,COUNTA(total)),3,1)')), {
      ...context,
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] => {
        if (start === 'A1' && end === 'A3') {
    ...
    expectArrayContains(averageResult, (value) => value.tag === ValueTag.Error && value.code === ErrorCode.Div0)
    expectArrayContains(defaultResult, (value) => value.tag === ValueTag.Number && value.value === 0)
    expectArrayContains(missingBuiltinResult, (value) => value.tag === ValueTag.Error && value.code === ErrorCode.Name)
    expectArrayContains(lambdaTotalResult, (value) => value.tag === ValueTag.Number && value.value === 2)
  })
  ...
  it('covers array-helper coercion failures that route through scalar and shape guards', () => {
    ...
    expect(evaluatePlan(lowerToPlan(parseFormula('TEXTSPLIT("a,b",",","",SEQUENCE(2))')), context)).toEqual({
      tag: ValueTag.Error,
      code: ErrorCode.Value,
    })
    expect(evaluatePlan(lowerToPlan(parseFormula('TEXTSPLIT("a,b",",","",TRUE(),SEQUENCE(2))')), context)).toEqual({
      tag: ValueTag.Error,
      code: ErrorCode.Value,
    })
    ...
    expect(evaluatePlan(lowerToPlan(parseFormula('TRIMRANGE(A1:B2,SEQUENCE(2))')), context)).toEqual({
      tag: ValueTag.Error,
      code: ErrorCode.Value,
    })
  })
...
function makeLookupExactMatchInstruction(overrides: Partial<ExactMatchInstruction> = {}): ExactMatchInstruction {
  return {
    opcode: 'lookup-exact-match',
    callee: 'MATCH',
    start: 'A1',
...
function makeLookupApproximateMatchInstruction(overrides: Partial<ApproximateMatchInstruction> = {}): ApproximateMatchInstruction {
  return {
    opcode: 'lookup-approximate-match',
    callee: 'MATCH',
    start: 'A1',
...
function withInvalidLookupCallee<T extends ExactMatchInstruction | ApproximateMatchInstruction>(instruction: T): T {
  Reflect.set(instruction, 'callee', 'NOPE')
  return instruction
}

function expectArrayContains(result: ReturnType<typeof evaluatePlanResult>, predicate: (value: CellValue) => boolean): void {
