...
describe('binder wasm rules', () => {
  ...
  it('keeps representative arity and wasm-argument policies stable', () => {
    ...
    expect(isWasmSafeBuiltinArity('ABS', 1)).toBe(true)
    expect(isWasmSafeBuiltinArity('ABS', 2)).toBe(false)
    expect(isWasmSafeBuiltinArity('POWER', 2)).toBe(true)
    expect(isWasmSafeBuiltinArity('POWER', 3)).toBe(false)
    expect(isWasmSafeBuiltinArity('ROUND', 2)).toBe(true)
    expect(isWasmSafeBuiltinArity('ROUND', 3)).toBe(false)
    expect(isWasmSafeBuiltinArity('LEFT', 2)).toBe(true)
    expect(isWasmSafeBuiltinArity('LEFT', 3)).toBe(false)
    expect(isWasmSafeBuiltinArity('LEN', 1)).toBe(true)
    expect(isWasmSafeBuiltinArity('LEN', 2)).toBe(false)
  ...
  it('covers promoted wasm argument-shape policies', () => {
    ...
    expectArgs('COUNTBLANK(A1:A4)', true)
    expectArgs('COUNTBLANK(A1)', true)
    expectArgs('COUNTBLANK("")', false)
    expectArgs('COUNTBLANK(A1,A2)', false)
    ...
    expectArgs('AVERAGE(A:B)', true)
    expectArgs('AVERAGE(A1:A4)', false)
    expectArgs('AVG(A1:A4)', false)
    expectArgs('AVG(SEQUENCE(2))', true)
    ...
    expectArgs('XLOOKUP("ID2",O1:O3,P1:R3)', true)
    expectArgs('XLOOKUP("Q2",J1:M1,J2:M4)', true)
  ...
  it('covers representative arity policy families', () => {
    const cases: Array<readonly [string, number, boolean]> = [
      ...
      ['SMALL', 2, true],
      ['SMALL', 3, false],
      ['PERCENTILE.INC', 2, true],
      ['PERCENTILE.INC', 3, false],
      ['INTERCEPT', 2, true],
      ['INTERCEPT', 3, false],
      ['RANK', 3, true],
      ['RANK', 4, false],
