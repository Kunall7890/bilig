...
const numericResultOrError = (value: number): CellValue => (Number.isFinite(value) ? numberResult(value) : valueError())
...
describe('numeric builtin helpers', () => {
  ...
  it('builds arithmetic helpers with injected coercion and error behavior', () => {
    ...
    expect(helpers.roundWith({ tag: ValueTag.Number, value: 12.34 }, { tag: ValueTag.Number, value: -1 })).toEqual({
      tag: ValueTag.Number,
      value: 10,
    })
    expect(helpers.floorWith({ tag: ValueTag.Number, value: 2 }, { tag: ValueTag.Number, value: 0 })).toEqual({
      tag: ValueTag.Error,
      code: ErrorCode.Div0,
    })
    ...
    expect(helpers.binaryMath({ tag: ValueTag.Number, value: 2 }, { tag: ValueTag.Boolean, value: true }, Math.pow)).toEqual({
      tag: ValueTag.Number,
      value: 2,
    })
  })
