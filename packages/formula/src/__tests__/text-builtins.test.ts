...
describe('text builtins', () => {
  ...
  it('supports Excel-like SEARCH wildcards and FIND case sensitivity', () => {
    ...
    expect(getTextBuiltin('FIND')?.(text('A'), text('abc'), err(ErrorCode.Ref))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('SEARCH')?.(text('A'), text('abc'), err(ErrorCode.Name))).toEqual(err(ErrorCode.Name))
  })
  ...
  it('supports ENCODEURL for URL-safe formatting', () => {
    expect(getTextBuiltin('ENCODEURL')?.(text('https://example.com/a b'))).toEqual(text('https://example.com/a%20b'))
  ...
  it('supports FINDB, LEFTB, MIDB, and RIGHTB with byte boundaries', () => {
    ...
    expect(getTextBuiltin('REPLACEB')?.(text('alphabet'), number(3), number(2), text('Z'))).toEqual(text('alZabet'))
  })
  ...
  it('validates FINDB/LEFTB/MIDB/RIGHTB argument bounds as VALUE errors', () => {
    ...
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(0), number(1), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(1), number(-1), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(1), text('bad'), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(1), number(1))).toEqual(valueError())
    expect(getTextBuiltin('REPLACEB')?.(err(ErrorCode.Ref), number(1), number(1), text('z'))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('LEFTB')?.(err(ErrorCode.Ref), number(1))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('MIDB')?.(text('abc'), err(ErrorCode.Name), number(1))).toEqual(err(ErrorCode.Name))
    expect(getTextBuiltin('RIGHTB')?.(text('abc'), err(ErrorCode.Div0))).toEqual(err(ErrorCode.Div0))
  ...
  it('supports TEXTJOIN delimiter, ignore_empty behavior, and error propagation', () => {
    ...
    expect(TEXTJOIN(text(','), { tag: ValueTag.Boolean, value: false }, text('a'), text('b'))).toEqual(text('a,b'))
    expect(TEXTJOIN(text('|'), { tag: ValueTag.Boolean, value: false }, text('a'), text(''), text('b'))).toEqual(text('a||b'))
    expect(TEXTJOIN(text('|'), { tag: ValueTag.Boolean, value: true }, undefined, text('a'))).toEqual(text('a'))
    expect(TEXTJOIN(text('|'), text('bad'), text('a'))).toEqual(valueError())
    expect(TEXTJOIN(text('|'), { tag: ValueTag.Boolean, value: false }, text('a'), { tag: ValueTag.Empty }, text('b'))).toEqual(
      text('a||b'),
    )
    expect(TEXTJOIN(text('|'), { tag: ValueTag.Boolean, value: false }, text('a'), text(''), text(''))).toEqual(text('a||'))
    expect(TEXTJOIN(text('|'), { tag: ValueTag.Boolean, value: false }, text('a'), err(ErrorCode.Ref), text('b'))).toEqual(
      err(ErrorCode.Ref),
    )
    expect(TEXTJOIN(text('-'), { tag: ValueTag.Boolean, value: true }, text('a'), text(''), text('b'))).toEqual(text('a-b'))
    expect(TEXTJOIN(text('-'), { tag: ValueTag.Boolean, value: true }, text('a'), text(''), text(''))).toEqual(text('a'))
  ...
  it('supports REGEXTEST, REGEXREPLACE, and REGEXEXTRACT modes', () => {
    ...
    expect(REGEXREPLACE(text('a1 b2 c3'), text('[0-9]'), text('X'), number(2))).toEqual(text('a1 bX c3'))
    expect(REGEXREPLACE(text('abc123'), text('([a-z]+)([0-9]+)'), text('$2-$1'))).toEqual(text('123-abc'))
  ...
  it('covers remaining VALUETOTEXT and regex validation branches', () => {
    ...
    expect(REGEXREPLACE(text('abc'), text('a'), text('x'), number(1), number(9))).toEqual(valueError())
    ...
    expect(REGEXREPLACE(text('abc123'), text('([a-z]+)([0-9]+)'), text('$2-$1'), number(3))).toEqual(text('abc123'))
    expect(REGEXREPLACE(text('abc'), text('([a-z]+)([0-9]+)?'), text('$2-$1'), number(1))).toEqual(text('-abc'))
    expect(REGEXREPLACE(text('a1 b2 c3'), text('[0-9]'), text('X'), number(-1))).toEqual(text('a1 b2 cX'))
  ...
  it('covers remaining case-conversion and byte-search validation paths', () => {
    ...
    expect(getTextBuiltin('SEARCHB')?.(err(ErrorCode.Ref), text('alphabet'))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('SEARCHB')?.(text('a'), err(ErrorCode.Name))).toEqual(err(ErrorCode.Name))
    expect(getTextBuiltin('SEARCHB')?.(text('é'), text('café'))).toEqual(number(4))
  })

  it('covers scalar text builtin validation and boundary branches', () => {
    expect(getTextBuiltin('LENB')?.()).toEqual(valueError())
    expect(getTextBuiltin('LENB')?.({ tag: ValueTag.Empty })).toEqual(number(0))
    expect(getTextBuiltin('CHAR')?.()).toEqual(valueError())
    expect(getTextBuiltin('CHAR')?.(text('bad'))).toEqual(valueError())
    expect(getTextBuiltin('CHAR')?.(number(Number.POSITIVE_INFINITY))).toEqual(valueError())
    expect(getTextBuiltin('CODE')?.()).toEqual(valueError())
    expect(getTextBuiltin('UNICODE')?.()).toEqual(valueError())
    expect(getTextBuiltin('UNICODE')?.(text(''))).toEqual(valueError())
    expect(getTextBuiltin('UNICHAR')?.()).toEqual(valueError())
    expect(getTextBuiltin('UNICHAR')?.(text('bad'))).toEqual(valueError())
    expect(getTextBuiltin('UNICHAR')?.(number(Number.POSITIVE_INFINITY))).toEqual(valueError())
    expect(getTextBuiltin('UNICHAR')?.(number(0x110000))).toEqual(valueError())
    expect(getTextBuiltin('LEFT')?.()).toEqual(valueError())
    expect(getTextBuiltin('LEFT')?.(text('abc'))).toEqual(text('a'))
    expect(getTextBuiltin('LEFT')?.(text('abc'), number(-1))).toEqual(valueError())
    expect(getTextBuiltin('LEFT')?.(text('abc'), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('RIGHT')?.()).toEqual(valueError())
    expect(getTextBuiltin('RIGHT')?.(text('abc'), number(0))).toEqual(text(''))
    expect(getTextBuiltin('RIGHT')?.(text('abc'), text('bad'))).toEqual(valueError())
    expect(getTextBuiltin('RIGHT')?.(text('abc'), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('MID')?.(text('abc'), number(2))).toEqual(valueError())
    expect(getTextBuiltin('MID')?.(text('abc'), number(1), number(-1))).toEqual(valueError())
    expect(getTextBuiltin('MID')?.(text('abc'), number(1), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('LEFTB')?.(text('éclair'), number(20))).toEqual(text('éclair'))
    expect(getTextBuiltin('LEFTB')?.(text('abc'), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('MIDB')?.(text('abc'), number(8), number(2))).toEqual(text(''))
    expect(getTextBuiltin('MIDB')?.(text('abc'), number(2), number(0))).toEqual(text(''))
    expect(getTextBuiltin('MIDB')?.(text('abc'), number(1), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('RIGHTB')?.(text('abc'), number(0))).toEqual(text(''))
    expect(getTextBuiltin('RIGHTB')?.(text('abc'), number(-0.5))).toEqual(valueError())
    expect(getTextBuiltin('REPLACE')?.(text(''), number(1), number(0), text('z'))).toEqual(text('z'))
    expect(getTextBuiltin('REPLACE')?.(text('abc'), number(4), number(0), text('z'))).toEqual(text('abcz'))
    expect(getTextBuiltin('REPLACE')?.(text('abc'), number(8), number(1), text('z'))).toEqual(text('abcz'))
    expect(getTextBuiltin('REPLACE')?.()).toEqual(valueError())
    expect(getTextBuiltin('REPLACE')?.(err(ErrorCode.Ref), number(1), number(1), text('z'))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('REPLACE')?.(text('abc'), text('bad'), number(1), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACE')?.(text('abc'), number(1), number(-1), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACE')?.(text('abc'), number(1), number(-0.5), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('REPLACEB')?.(text(''), number(1), number(0), text('z'))).toEqual(text('z'))
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(4), number(0), text('z'))).toEqual(text('abcz'))
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(8), number(1), text('z'))).toEqual(text('abcz'))
    expect(getTextBuiltin('REPLACEB')?.(text('abc'), number(1), number(-0.5), text('z'))).toEqual(valueError())
    expect(getTextBuiltin('SUBSTITUTE')?.()).toEqual(valueError())
    expect(getTextBuiltin('SUBSTITUTE')?.(err(ErrorCode.Ref), text('a'), text('z'))).toEqual(err(ErrorCode.Ref))
    expect(getTextBuiltin('REPT')?.()).toEqual(valueError())
    expect(getTextBuiltin('REPT')?.(text('x'), text('bad'))).toEqual(valueError())
    expect(getTextBuiltin('REPT')?.(text('x'), number(-0.5))).toEqual(valueError())
  })
})
