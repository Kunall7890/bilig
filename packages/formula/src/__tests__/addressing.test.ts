...
describe('addressing helpers', () => {
  it('round-trips columns and detects reference text kinds', () => {
    ...
    expect(isCellReferenceText('XFD1048576')).toBe(true)
    expect(isCellReferenceText('XFE1')).toBe(false)
    expect(isCellReferenceText('CHANGE1')).toBe(false)
    expect(isCellReferenceText('A1048577')).toBe(false)
    expect(isColumnReferenceText('$ab')).toBe(true)
    expect(isColumnReferenceText('A1')).toBe(false)
    expect(isColumnReferenceText('XFD')).toBe(true)
    expect(isColumnReferenceText('XFE')).toBe(false)
    expect(isRowReferenceText('42')).toBe(true)
    expect(isRowReferenceText('1048576')).toBe(true)
    expect(isRowReferenceText('1048577')).toBe(false)
  ...
  it('rejects invalid cell and range addresses', () => {
    ...
    expect(() => parseCellAddress('XFE1')).toThrow('Invalid reference: XFE1')
    expect(() => parseCellAddress('A1048577')).toThrow('Invalid reference: A1048577')
    expect(() => parseRangeAddress('XFE1:XFE2')).toThrow('Invalid reference: XFE1')
    expect(() => parseRangeAddress('1048577:1048577')).toThrow('Invalid reference: 1048577')
  })
})
