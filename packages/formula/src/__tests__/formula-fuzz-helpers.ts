...
export const cellRangeReferenceArbitrary = fc
  .tuple(sheetQualifiedArbitrary, columnArbitrary, rowArbitrary, fc.integer({ min: 0, max: 1 }), fc.integer({ min: 0, max: 1 }))
...
const axisRangeArgumentArbitrary = fc.oneof(
  fc.tuple(fc.boolean(), columnArbitrary, sheetQualifiedArbitrary).map(([anchorColumn, column, sheetPrefix]) => {
    const ref = `${anchorColumn ? '$' : ''}${column}`
    return `${sheetPrefix}${ref}:${ref}`
  }),
  fc.tuple(fc.boolean(), rowArbitrary, sheetQualifiedArbitrary).map(([anchorRow, row, sheetPrefix]) => {
    const ref = `${anchorRow ? '$' : ''}${row}`
    return `${sheetPrefix}${ref}:${ref}`
  }),
)

export const structuralFormulaReferenceArbitrary = fc.oneof(cellReferenceArbitrary, cellRangeReferenceArbitrary)
...
export const validFormulaArbitrary = fc.oneof(
  ...
  fc
    .tuple(fc.constantFrom('SUM', 'MAX', 'MIN', 'PRODUCT'), fc.array(aggregateArgumentArbitrary, { minLength: 1, maxLength: 3 }))
    .map(([name, args]) => `${name}(${args.join(',')})`),
  fc.tuple(scalarArbitrary, scalarArbitrary, scalarArbitrary).map(([condition, truthy, falsy]) => `IF(${condition},${truthy},${falsy})`),
)

export function renameScopedFormulaArbitrary(oldSheetName: string, newSheetName: string): fc.Arbitrary<string> {
  const safeSheetChoices = ['Sheet1', 'My Sheet', 'Data', 'Archive'].filter((candidate) => candidate !== newSheetName)
  ...
  const renameScopedCellReferenceArbitrary = fc
    .tuple(fc.boolean(), columnArbitrary, fc.boolean(), rowArbitrary, renameScopedSheetQualifiedArbitrary)
    .map(([anchorColumn, column, anchorRow, row, sheetPrefix]) => {
      return `${sheetPrefix}${anchorColumn ? '$' : ''}${column}${anchorRow ? '$' : ''}${row}`
    })
  ...
  return fc.oneof(
    ...
    fc
      .tuple(renameScopedScalarArbitrary, fc.constantFrom('+', '-', '*', '/', '&'), renameScopedScalarArbitrary)
      .map(([left, operator, right]) => `${left}${operator}${right}`),
    fc
      .tuple(fc.constantFrom('SUM', 'MAX', 'MIN', 'PRODUCT'), fc.array(renameScopedScalarArbitrary, { minLength: 1, maxLength: 3 }))
      .map(([name, args]) => `${name}(${args.join(',')})`),
  )
}
...
export const evaluableFormulaArbitrary = fc.oneof(
  ...
  fc
    .tuple(evaluableNumericScalarArbitrary, fc.constantFrom('+', '-', '*', '/'), evaluableNumericScalarArbitrary)
    .map(([left, operator, right]) => `${left}${operator}${right}`),
  fc
    .tuple(evaluableNumericScalarArbitrary, fc.constantFrom('>', '>=', '<', '<=', '=', '<>'), evaluableNumericScalarArbitrary)
    .map(([left, operator, right]) => `${left}${operator}${right}`),
  fc
    ...
    .map(([name, args]) => `${name}(${args.join(',')})`),
  fc
    .tuple(evaluableTextScalarArbitrary, fc.oneof(evaluableTextScalarArbitrary, evaluableCellReferenceArbitrary))
  ...
  fc.oneof(evaluableTextScalarArbitrary, evaluableCellReferenceArbitrary).map((value) => `LEN(${value})`),
)
