...
describe('js evaluator workbook special calls', () => {
  ...
  it('evaluates whole-axis lookup tables and vectors with resident bounds', () => {
    const context = {
      sheetName: 'Data',
      resolveCell: (): CellValue => ({ tag: ValueTag.Empty }),
      resolveRange: (_sheetName: string, start: string, end: string, refKind: 'cells' | 'rows' | 'cols'): CellValue[] => {
        if (refKind === 'cols' && start === 'A' && end === 'B') {
          return [text('a'), number(10), text('b'), number(20), text('c'), number(30)]
        }
        if (refKind === 'cols' && start === 'A' && end === 'A') {
          return [text('a'), text('b'), text('c')]
        }
        if (refKind === 'rows' && start === '1' && end === '2') {
          return [text('a'), text('b'), number(10), number(20)]
        }
        if (refKind === 'rows' && start === '1' && end === '1') {
          return [text('a'), text('b'), text('c')]
        }
        return []
      },
    ...
    expect(evaluateAst(parseFormula('VLOOKUP("b",A:B,2,FALSE())'), context)).toEqual(number(20))
    expect(evaluateAst(parseFormula('HLOOKUP("b",1:2,2,FALSE())'), context)).toEqual(number(20))
    expect(evaluateAst(parseFormula('MATCH("b",A:A,0)'), context)).toEqual(number(2))
    expect(evaluateAst(parseFormula('XMATCH("b",1:1,0,-1)'), context)).toEqual(number(2))
  })

  it('keeps whole-axis lookup and shape validation failures stable', () => {
    const raggedColumnContext = {
      sheetName: 'Data',
      resolveCell: (): CellValue => ({ tag: ValueTag.Empty }),
      resolveRange: (_sheetName: string, start: string, end: string, refKind: 'cells' | 'rows' | 'cols'): CellValue[] => {
        if (refKind === 'cols' && start === 'A' && end === 'B') {
          return [text('a'), number(10), text('b')]
        }
        if (refKind === 'cols' && start === 'A' && end === 'A') {
    ...
    expect(evaluateAst(parseFormula('ROWS(A:A,1)'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('VLOOKUP(A1:A2,A:B,2,FALSE())'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('VLOOKUP("b",A:B,2,FALSE())'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('VLOOKUP("b",A:B,2,A1:A2)'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('MATCH("b",A:B,0)'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('XMATCH("b",A:A,0,A1:A2)'), raggedColumnContext)).toEqual(err(ErrorCode.Value))
  })

  it('evaluates reference INDEX selections against cells, rows, columns, and omitted indexes', () => {
    const context = {
      sheetName: 'Sheet1',
    ...
    expect(evaluateAst(parseFormula('INDEX(A1:B2,2,2)'), context)).toEqual(number(7))
    expect(evaluateAstResult(parseFormula('INDEX(A1:B2,0,0)'), context)).toEqual({
      kind: 'array',
      rows: 2,
      cols: 2,
      values: [number(2), number(3), number(5), number(7)],
    })
    expect(evaluateAstResult(parseFormula('INDEX(A1:B2,0,1)'), context)).toEqual({
      kind: 'array',
      rows: 2,
      cols: 1,
      values: [number(2), number(5)],
    })
    expect(evaluateAstResult(parseFormula('INDEX(A1:B2,2,0)'), context)).toEqual({
      kind: 'array',
      rows: 1,
      cols: 2,
      values: [number(5), number(7)],
    })
    expect(evaluateAst(parseFormula('INDEX(A1:B2,)'), context)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('INDEX(A1:B2,-1,1)'), context)).toEqual(err(ErrorCode.Ref))
    expect(evaluateAst(parseFormula('INDEX(A1:B2,1,A1:B2)'), context)).toEqual(err(ErrorCode.Value))
    expect(evaluateAst(parseFormula('INDEX(B1,1)'), context)).toEqual(number(3))
    expect(evaluateAst(parseFormula('INDEX(1:1,2)'), context)).toEqual(number(3))
    expect(evaluateAst(parseFormula('INDEX(A:A,2)'), context)).toEqual(number(5))
  })

  it('filters hidden rows for scalar, array, and omitted SUBTOTAL arguments', () => {
    const context = {
      sheetName: 'Sheet1',
      resolveCell: (_sheetName: string, address: string): CellValue => (address === 'A2' ? number(4) : number(2)),
      resolveRange: (_sheetName: string, start: string, end: string): CellValue[] =>
        start === 'A1' && end === 'A2' ? [number(2), number(4)] : [],
      isRowHidden: (_sheetName: string, rowIndex: number) => rowIndex === 1,
    }

    expect(evaluateAst(parseFormula('SUBTOTAL(109,5,A1:A2)'), context)).toEqual(number(7))
    expect(evaluateAst(parseFormula('SUBTOTAL(109,A2)'), context)).toEqual(number(0))
    expect(evaluateAst(parseFormula('SUBTOTAL(109,{1,2;3,4})'), context)).toEqual(number(10))
    expect(evaluateAst(parseFormula('SUBTOTAL(109,)'), context)).toEqual(number(0))
    expect(evaluateAst(parseFormula('SUBTOTAL(,A1)'), context)).toEqual(err(ErrorCode.Value))
  })
})
