...
describe('formula', () => {
  ...
  it('routes accelerated text-splitting formulas to the wasm path while keeping indirection helpers on JS', () => {
    ...
    expect(compileFormula('SPLIT(A1,",")')).toMatchObject({ mode: 0, producesSpill: true })
  ...
  it('compiles IF, IFERROR, IFNA, and NA onto the wasm-safe path alongside exact-parity logical formulas', () => {
    ...
    expect(compileFormula('COUNTIF(TAKE(A1:A4,3),">2")').mode).toBe(1)
    expect(compileFormula('SUMIF(TAKE(A1:A4,3),">2",TAKE(B1:B4,3))').mode).toBe(1)
    expect(compileFormula('SUMIFS(TAKE(C1:C4,3),TAKE(A1:A4,3),">2",TAKE(B1:B4,3),"x")').mode).toBe(1)
    expect(compileFormula('SUMPRODUCT(TAKE(A1:A4,3),TAKE(B1:B4,3))').mode).toBe(1)
  ...
  it('keeps unsupported candidate builtin arities on the JS path', () => {
    ...
    expect(compileFormula('XLOOKUP("pear",A1:A3,B1:B3)')).toMatchObject({ mode: 1, producesSpill: false })
    expect(compileFormula('XLOOKUP("ID2",O1:O3,P1:R3)')).toMatchObject({ mode: 1, producesSpill: true })
    expect(compileFormula('XLOOKUP("Q2",J1:M1,J2:M4)')).toMatchObject({ mode: 1, producesSpill: true })
    ...
    expect(compileFormula('FLOOR(A1)').mode).toBe(0)
    expect(compileFormula('FLOOR(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('CEILING(A1)').mode).toBe(0)
    expect(compileFormula('CEILING(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('PI(A1)').mode).toBe(0)
    expect(compileFormula('ISBLANK()').mode).toBe(0)
    expect(compileFormula('ISNUMBER()').mode).toBe(0)
    expect(compileFormula('ISTEXT()').mode).toBe(0)
    expect(compileFormula('T()').mode).toBe(0)
    expect(compileFormula('N()').mode).toBe(0)
    expect(compileFormula('TYPE()').mode).toBe(0)
    ...
    expect(compileFormula('BITAND(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('BITOR(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('BITXOR(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('BITLSHIFT(A1,A2,A3)').mode).toBe(0)
    expect(compileFormula('BITRSHIFT(A1,A2,A3)').mode).toBe(0)
