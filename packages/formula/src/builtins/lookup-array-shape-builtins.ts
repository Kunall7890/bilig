...
export function createLookupArrayShapeBuiltins(deps: LookupArrayShapeBuiltinDeps): Record<string, LookupBuiltin> {
  return {
    ...
    OFFSET: (referenceArg, rowsArg, colsArg, heightArg, widthArg, areaNumberArg) => {
      ...
      if (rows === undefined || cols === undefined || height === undefined || width === undefined || areaNumber === undefined) {
        return deps.errorValue(ErrorCode.Value)
      }
      ...
      if (rowStart < 0 || colStart < 0 || rowStart + height > reference.rows || colStart + width > reference.cols) {
        return deps.errorValue(ErrorCode.Ref)
      }
      if (height === 1 && width === 1) {
    ...
    TAKE: (arrayArg, rowsArg, colsArg) => {
      ...
      const rowCount = clippedRows > 0 ? Math.min(clippedRows, array.rows) : Math.min(-clippedRows, array.rows)
      const colCount = clippedCols > 0 ? Math.min(clippedCols, array.cols) : Math.min(-clippedCols, array.cols)
    ...
    DROP: (arrayArg, rowsArg, colsArg) => {
      ...
      const dropRows = clippedRows >= 0 ? Math.min(clippedRows, array.rows) : Math.min(-clippedRows, array.rows)
      const dropCols = clippedCols >= 0 ? Math.min(clippedCols, array.cols) : Math.min(-clippedCols, array.cols)
    ...
    WRAPROWS: (arrayArg, wrapCountArg, padWithArg, padByColArg) => {
      ...
      const padValue: CellValue = padWithArg === undefined ? deps.errorValue(ErrorCode.NA) : padWithArg
    ...
    WRAPCOLS: (arrayArg, wrapCountArg, padWithArg, padByColArg) => {
      ...
      const padValue: CellValue = padWithArg === undefined ? deps.errorValue(ErrorCode.NA) : padWithArg
      const paddedValues = Array.from({ length: rows * cols }, (_, index) => values[index] ?? padValue)
