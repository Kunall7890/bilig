...
function regressionMatrixFromArg(
  ...
  if (!isRangeArg(arg)) {
    ...
    return numeric === undefined ? errorValue(ErrorCode.Value) : { values: [numeric], rows: 1, cols: 1 }
  }
...
function forecastResult(
  ...
  return 'intercept' in regression ? deps.numberResult(regression.intercept + regression.slope * x) : regression
}
...
function trendLikeResult(
  ...
  const knownXMatrix =
    knownXArg === undefined ? defaultRegressionSequence(knownYMatrix.rows, knownYMatrix.cols) : regressionMatrixFromArg(knownXArg, deps)
  if (!('values' in knownXMatrix)) {
    return knownXMatrix
  }
  if (knownXMatrix.values.length !== knownYMatrix.values.length || knownXMatrix.values.length === 0) {
    return deps.errorValue(ErrorCode.Value)
  }
...
function parseUnivariateRegressionDataset(
  ...
  const knownY =
    mode === 'logest' ? knownYMatrix.values.map((value) => (value > 0 ? Math.log(value) : Number.NaN)) : [...knownYMatrix.values]
...
function regressionStatCell(value: number | undefined, { errorValue, numberResult }: LookupRegressionBuiltinDeps): CellValue {
...
function linearEstimationResult(
  ...
  const trailing = mode === 'logest' ? (includeIntercept ? Math.exp(regression.intercept) : 1) : includeIntercept ? regression.intercept : 0
...
export function createLookupRegressionBuiltins(deps: LookupRegressionBuiltinDeps): Record<string, LookupBuiltin> {
  return {
    ...
    RSQ: (knownYArg, knownXArg) => {
      ...
      const correlation = correlationFromPairs(values.first, values.second, deps)
      return typeof correlation === 'number' ? deps.numberResult(correlation * correlation) : correlation
    },
