...
function regularizedBeta(x: number, alpha: number, beta: number): number {
  if (!Number.isFinite(x) || !Number.isFinite(alpha) || !Number.isFinite(beta) || x < 0 || x > 1 || alpha <= 0 || beta <= 0) {
    return Number.NaN
  }
  if (x === 0) {
    return 0
...
function chiSquareTestResult(
  ...
  return Number.isFinite(probability) ? { tag: ValueTag.Number, value: probability } : deps.errorValue(ErrorCode.Value)
}

function fTestResult(firstArg: LookupBuiltinArgument, secondArg: LookupBuiltinArgument, deps: LookupHypothesisBuiltinDeps): CellValue {
  ...
  if (type === 1) {
    ...
    if (type === 2) {
      const pooledVariance =
        ((first.length - 1) * firstVariance + (second.length - 1) * secondVariance) / (first.length + second.length - 2)
      if (!(pooledVariance > 0)) {
        return deps.errorValue(ErrorCode.Div0)
      }
      statistic = (firstMean - secondMean) / Math.sqrt(pooledVariance * (1 / first.length + 1 / second.length))
      ...
      const welchDenominator = (firstTerm * firstTerm) / (first.length - 1) + (secondTerm * secondTerm) / (second.length - 1)
...
export function createLookupHypothesisBuiltins(deps: LookupHypothesisBuiltinDeps): Record<string, LookupBuiltin> {
  return {
    ...
    'F.TEST': (firstArg, secondArg) => {
      return firstArg === undefined || secondArg === undefined ? deps.errorValue(ErrorCode.Value) : fTestResult(firstArg, secondArg, deps)
    },
    FTEST: (firstArg, secondArg) => {
      return firstArg === undefined || secondArg === undefined ? deps.errorValue(ErrorCode.Value) : fTestResult(firstArg, secondArg, deps)
    },
    'Z.TEST': (arrayArg, xArg, sigmaArg) => {
      return arrayArg === undefined || xArg === undefined ? deps.errorValue(ErrorCode.Value) : zTestResult(arrayArg, xArg, deps, sigmaArg)
    },
    ZTEST: (arrayArg, xArg, sigmaArg) => {
      return arrayArg === undefined || xArg === undefined ? deps.errorValue(ErrorCode.Value) : zTestResult(arrayArg, xArg, deps, sigmaArg)
    },
    'T.TEST': (firstArg, secondArg, tailsArg, typeArg) => {
      return firstArg === undefined || secondArg === undefined || tailsArg === undefined || typeArg === undefined
        ? deps.errorValue(ErrorCode.Value)
