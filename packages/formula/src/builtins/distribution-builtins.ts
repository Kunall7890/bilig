...
export function createDistributionBuiltins({
  ...
  const builtins: Record<string, Builtin> = {
    ...
    'CHISQ.DIST': (xArg, degreesArg, cumulativeArg) => {
      ...
      const degrees = toNumber(degreesArg)
      const cumulative = coerceBoolean(cumulativeArg, false)
      if (x === undefined || degrees === undefined || cumulative === undefined || x < 0 || degrees < 1) {
        return valueError()
      }
    ...
    'CHISQ.INV.RT': (probabilityArg, degreesArg) => {
      const probability = toNumber(probabilityArg)
      const degrees = toNumber(degreesArg)
      if (probability === undefined || degrees === undefined || !(probability > 0 && probability < 1) || !(degrees >= 1)) {
        return valueError()
      }
    ...
    'F.DIST': (xArg, degrees1Arg, degrees2Arg, cumulativeArg) => {
      ...
      const cumulative = coerceBoolean(cumulativeArg, false)
      if (x === undefined || degrees1 === undefined || degrees2 === undefined || cumulative === undefined) {
        return valueError()
      }
    ...
    'T.INV.2T': (probabilityArg, degreesArg) => {
      const probability = toNumber(probabilityArg)
      const degrees = integerValue(degreesArg)
      if (probability === undefined || degrees === undefined || !(probability > 0 && probability < 1)) {
        return valueError()
      }
    ...
    HYPGEOMDIST: (sampleSuccessesArg, sampleSizeArg, populationSuccessesArg, populationSizeArg) => {
      ...
      if (sampleSuccesses === undefined || sampleSize === undefined || populationSuccesses === undefined || populationSize === undefined) {
        return valueError()
      }
    ...
    NEGBINOMDIST: (failuresArg, successesArg, probabilityArg) => {
      ...
      const probability = toNumber(probabilityArg)
      if (failures === undefined || successes === undefined || probability === undefined || probability < 0 || probability > 1) {
        return valueError()
      }
