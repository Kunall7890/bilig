export function futureValue(rate: number, periods: number, payment: number, present: number, type: number): number {
  if (rate === 0) {
...
export function presentValue(rate: number, periods: number, payment: number, future: number, type: number): number {
  if (rate === 0) {
...
export function periodicPayment(rate: number, periods: number, present: number, future: number, type: number): number | undefined {
  if (periods <= 0) {
    return undefined
  }
  if (rate === 0) {
...
export function totalPeriods(rate: number, payment: number, present: number, future: number, type: number): number | undefined {
  if (payment === 0 && rate === 0) {
    return undefined
  }
  if (rate === 0) {
...
function annuityRateEquation(rate: number, periods: number, payment: number, present: number, future: number, type: number): number {
...
export function solveRate(
  ...
  for (let iteration = 0; iteration < 50; iteration += 1) {
    ...
    let nextRate: number
    if (!Number.isFinite(previousError) || !Number.isFinite(currentError) || currentError === previousError) {
      const epsilon = Math.max(1e-7, Math.abs(currentRate) * 1e-7)
      const forward = annuityRateEquation(currentRate + epsilon, periods, payment, present, future, type)
      const backward = annuityRateEquation(currentRate - epsilon, periods, payment, present, future, type)
...
function fixedDecliningBalanceRate(cost: number, salvage: number, life: number): number | undefined {
  if (!Number.isFinite(cost) || !Number.isFinite(salvage) || !Number.isFinite(life) || cost <= 0 || salvage < 0 || life <= 0) {
    return undefined
  }
...
export function ddbDepreciation(cost: number, salvage: number, life: number, period: number, factor: number): number | undefined {
  if (
  ...
  while (current < period && bookValue > salvage) {
    ...
    const full = Math.min(Math.max((bookValue * factor) / life, 0), Math.max(0, bookValue - salvage))
...
export function cumulativePeriodicPayment(
  ...
  if (rate <= 0 || periods <= 0 || present <= 0 || startPeriod < 1 || endPeriod < startPeriod || endPeriod > periods) {
    return undefined
  }
