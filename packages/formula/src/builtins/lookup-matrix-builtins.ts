...
function sumOfNumbers(arg: LookupBuiltinArgument | undefined, deps: LookupMatrixBuiltinDeps): number | CellValue {
  if (arg === undefined) {
...
export function createLookupMatrixBuiltins(deps: LookupMatrixBuiltinDeps): Record<string, LookupBuiltin> {
  return {
    SUMPRODUCT: (...args) => {
      if (args.length === 0) {
        return deps.errorValue(ErrorCode.Value)
