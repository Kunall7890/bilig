  ...
  it('preserves workbook semantics and metadata across JSON wire serialization boundaries', async () => {
    await runProperty({
      ...
      predicate: async ({ seedName, actions }) => {
        const seedSnapshot = await createEngineSeedSnapshot(seedName, `fuzz-snapshot-wire-${seedName}`)
...
async function roundtripViaWire(snapshot: WorkbookSnapshot, suffix: string): Promise<WorkbookSnapshot> {
