...
describe('replica-state', () => {
  ...
  it('retains recreated sheets and newer cell writes after tombstones', () => {
    ...
    expect(compactLog([createSheet, firstWrite, deleteSheet, recreateSheet, secondWrite])).toEqual([recreateSheet, secondWrite])
  })
  ...
  it('compacts workbook metadata entities by logical key', () => {
    ...
    expect(compactLog([define, redefine, pivotCreate, pivotDelete])).toEqual([redefine, pivotDelete])
  })
  ...
  it('drops stale row and range metadata that sit behind a later sheet tombstone', () => {
    ...
    expect(compactLog([createSheet, rowMetadata, styleRange, formatRange, deleteSheet])).toEqual([deleteSheet])
  })

  it('keeps global style and number format entities while compacting them by id', () => {
    const state = createReplicaState('replica')
    const firstStyle = createBatch(state, [{ kind: 'upsertCellStyle', style: { id: 'style:body', fill: { backgroundColor: '#fff' } } }])
    const latestStyle = createBatch(state, [{ kind: 'upsertCellStyle', style: { id: 'style:body', fill: { backgroundColor: '#eee' } } }])
    const firstFormat = createBatch(state, [{ kind: 'upsertCellNumberFormat', format: { id: 'fmt:body', code: '0.0', kind: 'number' } }])
    const latestFormat = createBatch(state, [{ kind: 'upsertCellNumberFormat', format: { id: 'fmt:body', code: '0.00', kind: 'number' } }])

    expect(compactLog([firstStyle, latestStyle, firstFormat, latestFormat])).toEqual([latestStyle, latestFormat])
  })
  ...
  it('compacts media entities by normalized id and drops sheet-barriered upserts', () => {
    ...
    const createDashboard = createBatch(state, [{ kind: 'upsertSheet', name: 'Dashboard', order: 1 }])
    ...
    expect(compareOpOrder(batchOpOrder(chartDelete, 0), batchOpOrder(imageDelete, 0))).toBeLessThan(0)
  })
