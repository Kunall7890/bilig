...
async function roundtripSnapshot(snapshot: WorkbookSnapshot, suffix: string): Promise<WorkbookSnapshot> {
  ...
  it('preserves semantic parity across repeated snapshot restore cycles', async () => {
    await runProperty({
      ...
      predicate: async ({ seedName, actions }) => {
        const seedSnapshot = await createEngineSeedSnapshot(seedName, `fuzz-core-snapshot-${seedName}`)
        ...
        const accepted = await actions.reduce<Promise<Array<(typeof actions)[number]>>>(async (acceptedPromise, action) => {
        ...
        expect(projectMetadataSnapshot(finalSnapshot)).toEqual(projectMetadataSnapshot(expectedSnapshot))
