...
describe('EngineRecalcService', () => {
  ...
  it('recalculates AGGREGATE when row visibility metadata changes', async () => {
    const engine = new SpreadsheetEngine({ workbookName: 'recalc-aggregate-row-visibility' })
    await engine.ready()
    engine.createSheet('Sheet1')
    engine.setCellValue('Sheet1', 'A1', 1)
    engine.setCellValue('Sheet1', 'A2', 2)
    engine.setCellValue('Sheet1', 'A3', 3)
    engine.setCellFormula('Sheet1', 'B1', 'AGGREGATE(9,5,A1:A3)')
