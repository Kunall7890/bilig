...
describe('workbook style format store', () => {
  it('interns normalized styles and number formats', () => {
    ...
    const style = internCellStyle(catalog, { fill: { backgroundColor: '#abc' }, font: { bold: true } }, 'style-0')
    upsertCellStyle(catalog, style, () => {})

    expect(internCellStyle(catalog, { fill: { backgroundColor: '#aabbcc' }, font: { bold: true } }, 'style-0')).toEqual(style)
