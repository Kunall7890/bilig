...
describe('workbook metadata records', () => {
  it('clones defined names, properties, tables, filters, and sorts without leaking mutation', () => {
    ...
    expect(cloneDefinedNameValue({ kind: 'structured-ref', tableName: 'Revenue', columnName: 'Sales' })).toEqual({
      kind: 'structured-ref',
      tableName: 'Revenue',
      columnName: 'Sales',
    })
  ...
  it('builds canonical metadata keys and validates required ids', () => {
    expect(filterKey('Sheet1', { sheetName: 'Sheet1', startAddress: 'C3', endAddress: 'A1' })).toBe('Sheet1:A1:C3')
    expect(sortKey('Sheet1', { sheetName: 'Sheet1', startAddress: 'D4', endAddress: 'B2' })).toBe('Sheet1:B2:D4')
    expect(dataValidationKey('Sheet1', { sheetName: 'Sheet1', startAddress: 'D4', endAddress: 'B2' })).toBe('Sheet1:B2:D4')
  ...
  it('deletes and rekeys workbook metadata records across record families', () => {
    ...
    const expectSingleRekey = <T extends object>(bucket: Map<string, T>, rewrite: (record: T) => T, expectedKey: string) => {
