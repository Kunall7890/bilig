...
const sheetProtectionActionArbitrary = fc.boolean().map((hideFormulas) => ({ kind: 'setSheetProtection' as const, hideFormulas }))
...
export const snapshotSemanticActionArbitrary = fc.oneof<EngineSemanticAction>(coreReplicaActionArbitrary, metadataActionArbitrary)
