...
function compareBatches(left: EngineOpBatch, right: EngineOpBatch): number {
  return left.clock.counter - right.clock.counter || left.replicaId.localeCompare(right.replicaId) || left.id.localeCompare(right.id)
}
...
function sheetDeleteBarrierForOp(op: EngineOp, latestSheetDeletes: Map<string, OpOrder>): OpOrder | undefined {
  switch (op.kind) {
    ...
    case 'upsertChart':
      return latestSheetDeletes.get(op.chart.sheetName) ?? latestSheetDeletes.get(op.chart.source.sheetName)
    case 'deleteChart':
      return undefined
    case 'upsertImage':
