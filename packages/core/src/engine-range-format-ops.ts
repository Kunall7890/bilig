...
export function buildStylePatchOps(workbook: WorkbookStore, range: CellRangeRef, patch: CellStylePatch): EngineOp[] {
...
export function buildStyleClearOps(workbook: WorkbookStore, range: CellRangeRef, fields?: readonly CellStyleField[]): EngineOp[] {
  return materializeStyleRangeOps(workbook, range, (baseStyle) => workbook.internCellStyle(clearStyleFields(baseStyle, fields)))
}
...
export function buildFormatPatchOps(workbook: WorkbookStore, range: CellRangeRef, format: CellNumberFormatInput): EngineOp[] {
  const normalized = workbook.internCellNumberFormat(
    typeof format === 'string' ? format : createCellNumberFormatRecord(WorkbookStore.defaultFormatId, format),
  )
