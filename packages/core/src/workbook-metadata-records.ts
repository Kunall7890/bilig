...
export function cloneDefinedNameValue(value: WorkbookDefinedNameValueSnapshot): WorkbookDefinedNameValueSnapshot {
...
export function cloneDataValidationRule(rule: WorkbookDataValidationRuleSnapshot): WorkbookDataValidationRuleSnapshot {
  switch (rule.kind) {
    case 'list': {
...
export function cloneDataValidationRecord(record: WorkbookDataValidationRecord): WorkbookDataValidationRecord {
...
export function cloneConditionalFormatRule(rule: WorkbookConditionalFormatRuleSnapshot): WorkbookConditionalFormatRuleSnapshot {
  switch (rule.kind) {
    case 'cellIs':
      return {
        kind: 'cellIs',
...
function isCommentThreadRecord(record: unknown): record is WorkbookCommentThreadRecord {
  return typeof record === 'object' && record !== null && 'threadId' in record && 'comments' in record
}
...
function isTableRecord(record: unknown): record is WorkbookTableRecord {
  return typeof record === 'object' && record !== null && 'name' in record && 'columnNames' in record
}
...
function isImageRecord(record: unknown): record is WorkbookImageRecord {
  return (
    typeof record === 'object' && record !== null && 'id' in record && 'sheetName' in record && 'address' in record && 'sourceUrl' in record
  )
}

function isShapeRecord(record: unknown): record is WorkbookShapeRecord {
  return (
    typeof record === 'object' && record !== null && 'id' in record && 'sheetName' in record && 'address' in record && 'shapeType' in record
  )
}
