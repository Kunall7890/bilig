...
function getColumnCacheKey(sheetName: string, col: number, rowStart: number, rowEnd: number): string {
  return `${sheetName}\t${col}\t${rowStart}\t${rowEnd}`
}

function normalizeApproximateComparableValue(value: CellValue, lookupString: (id: number) => string, stringId = 0): ApproximateComparable {
  switch (value.tag) {
    case ValueTag.Empty:
      return { kind: 'empty' }
...
function resolveSingleColumnBounds(
  request: Pick<ApproximateVectorMatchRequest, 'sheetName' | 'start' | 'end' | 'startRow' | 'endRow' | 'startCol' | 'endCol'>,
