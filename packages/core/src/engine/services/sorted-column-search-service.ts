...
import {
  columnRegistryKey,
  compareApproximateNumeric,
  compareApproximateText,
  decodeValueTag,
  detectRepeatedUniformNumericStep,
  detectUniformNumericStep,
  findRepeatedUniformNumericApproximatePosition,
  findUniformNumericApproximatePosition,
  getColumnCacheKey,
  normalizeApproximateComparableValue,
  resolveSingleColumnBounds,
  supportsPreparedApproximateKind,
  type ApproximateColumnIndexEntry,
  type ApproximateComparable,
} from './sorted-column-search-helpers.js'
...
export interface SortedColumnSearchService {
  ...
  readonly findVectorMatch: (request: ApproximateVectorMatchRequest) => ApproximateVectorMatchResult
...
export function createSortedColumnSearchService(args: {
  ...
  const replaceColumnIndex = (cacheKey: string, entry: ApproximateColumnIndexEntry | undefined): void => {
  ...
  const buildApproximateColumnIndex = (sheetName: string, col: number, rowStart: number, rowEnd: number): ApproximateColumnIndexEntry => {
    ...
    const numericValues = Float64Array.from(rawValues, (value) => (value.kind === 'numeric' ? value.value : 0))
    let sortedAscending = true
    let sortedDescending = true
    for (let index = 1; index < numericValues.length; index += 1) {
      const comparison = compareApproximateNumeric(numericValues[index - 1]!, numericValues[index]!)
      if (comparison > 0) {
        sortedAscending = false
      }
      if (comparison < 0) {
  ...
  const ensureColumnIndex = (sheetName: string, col: number, rowStart: number, rowEnd: number): ApproximateColumnIndexEntry => {
  ...
  const updateEntryLiteralWrite = (
    ...
    if (
      (entry.comparableKind === 'numeric' && newValue.kind !== 'numeric' && newValue.kind !== 'empty') ||
  ...
  return {
    ...
    recordLiteralWrite(request) {
      ...
      const oldComparable = normalizeApproximateComparableValue(request.oldValue, (id) => args.state.strings.get(id), request.oldStringId)
      const newComparable = normalizeApproximateComparableValue(request.newValue, (id) => args.state.strings.get(id), request.newStringId)
      for (const cacheKey of cacheKeys) {
