...
export function createEngineFormulaGraphService(args: {
  ...
  const syncWasmProgramsNow = (wasmFormulaCount = countWasmFastPathFormulas()): void => {
    ...
    for (let rangeIndex = 0; rangeIndex < args.state.ranges.size; rangeIndex += 1) {
      ...
      args.getWasmRangeRowCounts()[rangeIndex] = descriptor.refCount > 0 ? descriptor.row2 - descriptor.row1 + 1 : 0
      args.getWasmRangeColCounts()[rangeIndex] = descriptor.refCount > 0 ? descriptor.col2 - descriptor.col1 + 1 : 0
    }
