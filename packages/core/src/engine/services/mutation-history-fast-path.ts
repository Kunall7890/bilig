...
export function tryBuildFastMutationHistory(args: FastMutationHistoryArgs): FastMutationHistoryResult | null {
  const forwardOps = Array<FastHistoryOp>(args.ops.length)
  if (args.preparedCellAddressesByOpIndex && args.preparedCellAddressesByOpIndex.length !== args.ops.length) {
  ...
  for (let index = forwardOps.length - 1; index >= 0; index -= 1) {
    ...
    if (
      (op.kind === 'setCellValue' || op.kind === 'setCellFormula' || op.kind === 'clearCell' || op.kind === 'setCellFormat') &&
      createdSheetNames.has(op.sheetName)
    ) {
      continue
    }
    const inverse = buildFastInverseOp(args.workbook, args.getCellByIndex, op, args.preparedCellAddressesByOpIndex?.[index] ?? null)
    if (!inverse) {
      return null
    }
