...
export interface SpreadsheetEngineSortRangeOptions {
  ...
  readonly recordSortMetadata?: boolean
}
...
export function buildSortRangeOps(
  ...
  if (options.recordSortMetadata !== false) {
...
export function buildSortTableOps(
  snapshot: WorkbookSnapshot,
  sheetName: string,
  tableName: string,
  keys: readonly WorkbookSortSnapshot['keys'][number][],
  ...
  if (keys.length === 0) {
    throw new Error('sortTable requires at least one sort key')
  }
  ...
  const tableBounds = sortBounds(sheetName, { sheetName, startAddress: table.startAddress, endAddress: table.endAddress })
  const bodyStartRow = table.headerRow ? tableBounds.startRow + 1 : tableBounds.startRow
  const bodyEndRow = table.totalsRow ? tableBounds.endRow - 1 : tableBounds.endRow
  if (bodyStartRow > bodyEndRow) {
    return []
  }

  const bodyRange: CellRangeRef = {
    sheetName,
    startAddress: formatAddress(bodyStartRow, tableBounds.startCol),
    endAddress: formatAddress(bodyEndRow, tableBounds.endCol),
  }
  const ops = buildSortRangeOps(snapshot, sheetName, bodyRange, keys, { recordSortMetadata: false })
  ops.push({
    kind: 'upsertTable',
    table: {
      ...structuredClone(table),
      sortState: buildTableSortStateXml(sheetName, bodyRange, keys),
    },
  })
  return ops
}

function buildTableSortStateXml(sheetName: string, bodyRange: CellRangeRef, keys: readonly WorkbookSortSnapshot['keys'][number][]): string {
  const bodyBounds = sortBounds(sheetName, bodyRange)
  const ref = `${bodyRange.startAddress}:${bodyRange.endAddress}`
  const conditions = keys
    .map((key) => {
      const keyCol = parseCellAddress(key.keyAddress, sheetName).col
      const conditionRef = `${formatAddress(bodyBounds.startRow, keyCol)}:${formatAddress(bodyBounds.endRow, keyCol)}`
      const descending = key.direction === 'desc' ? ' descending="1"' : ''
      return `<sortCondition${descending} ref="${escapeXml(conditionRef)}"/>`
    })
    .join('')
  return `<sortState ref="${escapeXml(ref)}">${conditions}</sortState>`
}
