...
const initialSparseIntegerCapacity = 16
...
function canStoreInt16Number(value: number): boolean {
...
function binarySearchUint32Prefix(values: Uint32Array, length: number, target: number): number {
  let low = 0
  let high = length - 1
  while (low <= high) {
