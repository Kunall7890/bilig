...
function addWorksheetControlXml(sheetXml: string): string {
  const withNamespaces = sheetXml.replace(
    /<worksheet\b/u,
    `<worksheet xmlns:r="${officeRelationshipNamespace}" xmlns:xdr="${drawingMlSpreadsheetNamespace}" xmlns:x14="${spreadsheetControlNamespace}" xmlns:mc="${markupCompatibilityNamespace}" mc:Ignorable="x14"`,
  )
  return withNamespaces.replace('</worksheet>', `${worksheetControlsXml}</worksheet>`)
}
...
const vmlDrawingXml = [
  ...
  '<x:ClientData ObjectType="Button"><x:SizeWithCells/><x:Anchor>6, 15, 3, 7, 11, 9, 4, 16</x:Anchor>',
