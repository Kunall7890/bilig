...
interface ProjectedFreezePaneSemantics {
  rows: number
  cols: number
  topLeftCell?: string
  activePane?: string
...
function defaultFreezePaneActivePane(freezePane: ProjectedFreezePaneSemantics): string {
...
function defaultFreezePaneTopLeftCell(freezePane: ProjectedFreezePaneSemantics): string {
  return XLSX.utils.encode_cell({ r: freezePane.rows, c: freezePane.cols })
}

function projectFreezePaneSemantics(freezePane: ProjectedFreezePaneSemantics | undefined): ProjectedFreezePaneSemantics | undefined {
  if (!freezePane) {
    return undefined
  }
  return {
