...
import type { CellRangeRef, WorkbookAxisEntrySnapshot, WorkbookSortSnapshot } from '@bilig/protocol'
...
function rangeLabel(range: Pick<WorkbookAgentPreviewRange, 'sheetName' | 'startAddress' | 'endAddress'>) {
...
export function isWorkbookAgentStructuralCommandKind(kind: string): kind is WorkbookAgentStructuralCommand['kind'] {
  switch (kind) {
    case 'createSheet':
    case 'renameSheet':
    case 'deleteSheet':
...
export function isWorkbookAgentStructuralCommand(command: WorkbookAgentCommand): command is WorkbookAgentStructuralCommand {
  return isWorkbookAgentStructuralCommandKind(command.kind)
}

export function isWorkbookAgentStructuralCommandValue(value: unknown): value is WorkbookAgentStructuralCommand {
  if (!isRecord(value)) {
    return false
  }
...
export function isHighRiskWorkbookAgentStructuralCommand(command: WorkbookAgentStructuralCommand): boolean {
...
export function describeWorkbookAgentStructuralCommand(command: WorkbookAgentStructuralCommand): string {
  switch (command.kind) {
    case 'createSheet':
...
export function estimateWorkbookAgentStructuralCommandAffectedCells(command: WorkbookAgentStructuralCommand): number | null {
  switch (command.kind) {
    case 'createSheet':
    case 'renameSheet':
    case 'deleteSheet':
...
export function deriveWorkbookAgentStructuralCommandPreviewRanges(command: WorkbookAgentStructuralCommand): WorkbookAgentPreviewRange[] {
  switch (command.kind) {
    case 'createSheet':
    case 'renameSheet':
    case 'deleteSheet':
...
export function applyWorkbookAgentStructuralCommand(engine: SpreadsheetEngine, command: WorkbookAgentStructuralCommand): void {
  switch (command.kind) {
    case 'createSheet':
      engine.renderCommit([
        {
    ...
    case 'updateRowMetadata': {
      ...
      engine.updateRowMetadata(command.sheetName, command.startRow, command.count, resolved.height, resolved.hidden)
      return
    }
    case 'updateColumnMetadata': {
      const resolved = resolveColumnMetadataCommandState(engine, command)
      engine.updateColumnMetadata(command.sheetName, command.startCol, command.count, resolved.width, resolved.hidden)
      return
    }
  }
}
