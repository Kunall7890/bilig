...
import type { CellRangeRef, WorkbookConditionalFormatRuleSnapshot, WorkbookConditionalFormatSnapshot } from '@bilig/protocol'
...
export function isWorkbookAgentConditionalFormatCommandKind(kind: string): kind is WorkbookAgentConditionalFormatCommand['kind'] {
  return kind === 'upsertConditionalFormat' || kind === 'deleteConditionalFormat'
}

export function isWorkbookAgentConditionalFormatCommand(command: WorkbookAgentCommand): command is WorkbookAgentConditionalFormatCommand {
  return isWorkbookAgentConditionalFormatCommandKind(command.kind)
}

export function isWorkbookAgentConditionalFormatCommandValue(value: unknown): value is WorkbookAgentConditionalFormatCommand {
  if (!isRecord(value) || typeof value['kind'] !== 'string') {
    return false
  }
  switch (value['kind']) {
...
export function isHighRiskWorkbookAgentConditionalFormatCommand(_command: WorkbookAgentConditionalFormatCommand): boolean {
  return false
}

export function isWorkbookScopeConditionalFormatCommand(_command: WorkbookAgentConditionalFormatCommand): boolean {
  return false
}

export function describeWorkbookAgentConditionalFormatCommand(command: WorkbookAgentConditionalFormatCommand): string {
  switch (command.kind) {
    case 'upsertConditionalFormat':
...
export function estimateWorkbookAgentConditionalFormatCommandAffectedCells(command: WorkbookAgentConditionalFormatCommand): number {
  return countRangeCells(command.kind === 'upsertConditionalFormat' ? command.format.range : command.range)
}
...
export function deriveWorkbookAgentConditionalFormatCommandPreviewRanges(
  ...
  return [
    {
      ...normalizeRangeBounds(command.kind === 'upsertConditionalFormat' ? command.format.range : command.range),
      role: 'target',
    },
  ]
}
