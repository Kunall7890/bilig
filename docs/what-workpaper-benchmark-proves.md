...
`47/57` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
generated at `2026-05-16T04:49:22.641Z`.

The overall directional mean-ratio geomean is `0.6619772067028068`, and the
overall directional p95-ratio geomean is `0.6873818250368949`. Ratios below
...
scorecard records `42/57` workloads winning both mean and p95. The worst p95
holdout is `lookup-approximate-duplicates`, where the current WorkPaper-to-HyperFormula
p95 ratio is `2.41182863089937`. The honest claim is `47/57` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `47/57` mean wins against
...
> command, and an explicit p95 caveat. the claim is `47/57` mean wins for the
