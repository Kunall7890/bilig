...
`77/100` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
generated at `2026-05-17T08:11:00.665Z`.

The overall directional mean-ratio geomean is `0.6141307080612967`, and the
overall directional p95-ratio geomean is `0.6252876836416962`. Ratios below
...
holdout is `structural-append-formula-rows-small`, where the current
WorkPaper-to-HyperFormula p95 ratio is `3.936729924852059`. The honest claim is
`77/100` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `77/100` mean wins against
...
> command, and an explicit p95 caveat. the claim is `77/100` mean wins for the
