...
`48/57` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
The overall directional mean-ratio geomean is `0.6101287289771713`, and the
overall directional p95-ratio geomean is `0.6134045337046674`. Ratios below
...
holdout is `structural-insert-columns`, where the current WorkPaper-to-HyperFormula
p95 ratio is `1.849181514616265`. The honest claim is `48/57` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `48/57` mean wins against
...
> command, and an explicit p95 caveat. the claim is `48/57` mean wins for the
