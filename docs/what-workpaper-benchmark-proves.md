...
generated at `2026-05-22T08:55:02.332Z`.

The overall directional mean-ratio geomean is `0.362395561128425`, and the
overall directional p95-ratio geomean is `0.3783178813267213`. Ratios below
`1.0` mean WorkPaper is faster on that metric.

The headless leadership scorecard records `90/100` workloads winning both
...
added. The current headless leadership scorecard records `90/100` workloads
winning both mean and p95. The current worst p95 row is
`structural-append-formula-rows-large`, where the current
WorkPaper-to-HyperFormula p95 ratio is `1.421342616086373`. The honest claim is
...
> `90/100` mean+p95 wins against comparable HyperFormula-style headless
...
> `90/100` mean+p95 wins for the current comparable headless WorkPaper
