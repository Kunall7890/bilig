...
`78/100` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
generated at `2026-05-17T10:02:57.471Z`.

The overall directional mean-ratio geomean is `0.6111822027846606`, and the
overall directional p95-ratio geomean is `0.6296855697729614`. Ratios below
...
scorecard records `76/100` workloads winning both mean and p95. The worst p95
holdout is `aggregate-overlapping-sliding-window`, where the current
WorkPaper-to-HyperFormula p95 ratio is `1.9001316764273575`. The honest claim is
`78/100` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `78/100` mean wins against
...
> command, and an explicit p95 caveat. the claim is `78/100` mean wins for the
