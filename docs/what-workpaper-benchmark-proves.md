...
`72/100` mean-latency wins on scorecard-eligible comparable workloads. This is a
...
generated at `2026-05-16T21:37:09.873Z`.

The overall directional mean-ratio geomean is `0.7071933537655859`, and the
overall directional p95-ratio geomean is `0.6979827319598825`. Ratios below
...
scorecard records `70/100` workloads winning both mean and p95. The worst p95
holdout is `lookup-text-exact`, where the current
WorkPaper-to-HyperFormula p95 ratio is `22.99194632249502`. The honest claim is
`72/100` mean wins plus an
...
> bilig's WorkPaper benchmark currently records `72/100` mean wins against
...
> command, and an explicit p95 caveat. the claim is `72/100` mean wins for the
