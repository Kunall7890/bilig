...
The checked benchmark artifact currently says Bilig wins `76/100` comparable
workloads on mean latency against the HyperFormula-style baseline. It wins
`73/100` on both mean and p95.

The miss is not hidden: `single-formula-edit-recalc` is slower at p95 by
`5.426x`. Browser grid rendering is not part of this benchmark.
...
The current benchmark artifact says 76/100 mean-latency wins on comparable
