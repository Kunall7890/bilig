...
The checked benchmark artifact currently says Bilig wins `79/100` comparable
workloads on mean latency against the HyperFormula-style baseline. It wins
`78/100` on both mean and p95.

The miss is not hidden: `build-dense-literals-wide` is slower at p95 by
`3.047x`. Browser grid rendering is not part of this benchmark.
...
The current benchmark artifact says 79/100 mean-latency wins on comparable
