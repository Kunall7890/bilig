...
against HyperFormula-style comparable workloads, and `48/57` workloads winning
both mean and p95.

The caveat is intentionally visible: `single-formula-edit-recalc` is slower at
p95 by `2.024x`. Browser grid rendering is outside this benchmark.
...
mean-latency wins against HyperFormula-style comparable workloads, 48/57
