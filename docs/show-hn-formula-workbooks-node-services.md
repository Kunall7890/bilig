...
checked by this page is `@bilig/headless@0.16.28`.
...
The checked benchmark artifact currently says Bilig wins `74/100` comparable
workloads on mean latency against the HyperFormula-style baseline. It wins
`68/100` on both mean and p95.

The miss is not hidden: `build-mixed-content-large` is slower at p95 by
`2.933x`. Browser grid rendering is not part of this benchmark.
...
The current benchmark artifact says 74/100 mean-latency wins on comparable
