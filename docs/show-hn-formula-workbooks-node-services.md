...
The checked benchmark artifact currently says Bilig wins `98/100` comparable
workloads on mean latency against the HyperFormula-style baseline. It wins
`97/100` on both mean and p95.

The worst p95 row is not hidden: `aggregate-overlapping-sliding-window` is the current worst p95
row at `1.104x`. Browser grid rendering is not part of this benchmark.
...
The current benchmark artifact says 98/100 mean-latency wins on comparable
