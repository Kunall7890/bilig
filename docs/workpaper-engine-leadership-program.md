...
| Performance dominance | Directly comparable benchmark workloads | unified benchmark includes `100` scorecard-eligible comparable workloads, public and holdout lanes | HyperFormula has `20` current mean wins in the latest artifact | `WorkPaper` leads `80/100` overall mean workloads and `76/100` mean+p95 workloads; the worst p95 row is `structural-insert-columns-small` | turn mean wins into larger-margin mean+p95 wins without weakening benchmark gates |
...
`performance-leading`: WorkPaper is `80/100` on comparable mean workloads and
...
- `WorkPaper` now leads most of the expanded competitive engine scorecard:
  - `80/100` overall scorecard-eligible comparable mean workloads
