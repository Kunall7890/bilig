...
- that artifact currently shows WorkPaper leading `81/100` scorecard-eligible
  comparable mean workloads and `81/100` mean+p95 workloads
...
| Performance dominance | Directly comparable benchmark workloads | unified benchmark includes `100` scorecard-eligible comparable workloads, public and holdout lanes | HyperFormula has `19` current mean wins in the latest artifact | `WorkPaper` leads `81/100` overall mean workloads and `81/100` mean+p95 workloads; the worst p95 row is `single-formula-edit-recalc` | turn mean wins into larger-margin mean+p95 wins without weakening benchmark gates |
...
- `bilig` canonical formula production closure:
  ...
  - current `WorkPaper` expanded-matrix wins on this host:
    - build-dense-literals at `4.91x` faster
    - build-many-sheets at `3.45x` faster
    - single-edit-fanout at `1.10x` faster
    - range-read-dense at `1.18x` faster
    - lookup without column indexing at `1.64x` faster
    - lookup-approximate-sorted at `1.07x` faster
    - lookup-text-exact at `4.00x` faster
  - HyperFormula current expanded-matrix win range on this host: `1.05x` to `3.80x`
  - notable improvements from the latest tranches:
    ...
    - single-edit recalculation improved from `1.42x` slower to a `WorkPaper` win at `1.61x` faster after large fresh-workbook formula sets started bootstrapping the WASM kernel synchronously on Node/Bun, public mutation entrypoints stopped routing through the generic `Effect` wrapper, coordinate-native simple-cell mutation paths stopped reparsing `sheetName + A1` on the engine hot path, local-only headless engine instances stopped paying replica-version bookkeeping, and the internal visibility cache stopped keying hot-path comparisons by A1 strings
    - lookup without column indexing improved from `134.35x` slower to a `WorkPaper` win at `1.51x` faster after the headless full-workbook diff path was removed from ordinary edits, the mutation pipeline stopped paying per-op internal `Effect` crossings, and the internal visibility cache stopped formatting A1 strings for every changed cell
    - lookup with `useColumnIndex` improved from `134.35x` slower to near parity at `1.21x` slower after direct exact lookup, event-driven `WorkPaper` change tracking, eager index priming, literal-only direct initialization, trusted owned-op execution, synchronous internal engine wiring, thresholded fresh-workbook kernel bootstrap, cheaper public mutation dispatch, coordinate-native simple-cell mutation, and numeric-key visibility caching landed
    - lookup-approximate-sorted improved from `6.67x` slower to a `WorkPaper` win at `1.07x` faster after approximate `MATCH` / `XMATCH` vector shapes started routing through a direct binary-search lookup service instead of the generic evaluator range-materialization path
...
`performance-leading`: WorkPaper is `81/100` on comparable mean workloads and
`81/100` on mean+p95 workloads. The active program is now about larger margins,
...
- `WorkPaper` now leads most of the expanded competitive engine scorecard:
  - `81/100` overall scorecard-eligible comparable mean workloads
  - `81/100` overall scorecard-eligible comparable mean+p95 workloads
  - `59/73` public comparable mean workloads
  - `21/27` holdout comparable mean workloads
  - current artifact generated at `2026-05-17T01:27:00.888Z`
...
### Priority 1: Turn close performance wins into larger-margin wins

Based on the checked-in benchmark artifact, the most urgent directly comparable
performance lanes are now closest wins, not losses:

- `build-mixed-content`: WorkPaper mean ratio `0.9353922725978107`, confidence
  intervals overlap.
- `structural-insert-columns`: WorkPaper mean ratio `0.8642535647861759`.
- `aggregate-overlapping-sliding-window`: WorkPaper mean ratio
  `0.8335704735358855`, confidence intervals overlap.
- `build-many-sheets`: WorkPaper mean ratio `0.8007693406232433`.
...
This is now larger-margin hardening rather than a broad scorecard loss. The
current checked-in artifact has `lookup-text-exact` green on both mean and p95;
lookup work should protect that win while focusing on any fixture where the
ratio remains too close for a durable leadership claim.
...
- these are the closest current WorkPaper wins in the checked artifact; there
  are no current HyperFormula mean-red rows, but these lanes are not close to
  the active `10x` target
