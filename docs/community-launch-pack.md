...
- External open-source directory surface: the Open-Awesome submission for Bilig
  has a metadata refresh comment with the current `@bilig/headless@0.16.28`
...
- npm surface: `@bilig/headless@0.16.28`, with `12,568` downloads in the
...
- Do not route new evaluator traffic to the older public DEV copy until it is
  ...
  checked package evidence is `@bilig/headless@0.16.28`, `74/100` comparable
  mean wins, and `68/100` mean+p95 wins.
...
The `74/100` WorkPaper mean-win claim should become small, quotable artifacts:
...
3/ Current benchmark artifact: 74/100 comparable mean-latency rows are faster
than the HyperFormula-style baseline.

The ugly part is public too: build-mixed-content is the visible p95
...
The benchmark claim is intentionally narrow: 74/100 comparable mean-latency wins
against HyperFormula-style workloads, build-mixed-content is the
