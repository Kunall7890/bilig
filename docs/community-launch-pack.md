...
- External JavaScript spreadsheet comparison surface: the jsgrids listing was
  already merged, and a follow-up pull request now points the Bilig entry at the
  live npm quickstart while documenting `@bilig/headless/xlsx` import/export:
...
- External open-source directory surface: the Open-Awesome submission for Bilig
  has a metadata refresh comment with the current `@bilig/headless` package
...
- External MCP directory follow-up on May 18, 2026: Glama search returns Bilig
  WorkPaper with TypeScript, Developer Tools, Workplace & Productivity, and
  Remote attributes. The Dockerfile test build
  `019e3900-c65c-73b4-b500-e3af43d37d46` succeeded and was published as
  Glama release `0.21.1`. The public Schema and Overview pages now expose the seven
  file-backed tools, and the score page reports A-grade Tool Definition Quality
  for each tool. The same npm path was smoked locally with
  `@bilig/headless@0.21.1`, server `bilig-headless-workpaper`, 7 file-backed
  tools, and `Summary!B3` display value `60000`; Glama's JSON API can lag the
  HTML listing immediately after release:
...
- The serverless WorkPaper API example now exercises an AdonisJS-style
  `HttpContext` route pair in `examples/serverless-workpaper-api/framework-adapters.ts`:
  one route reads `/api/workpaper/summary`, and one route posts
  `/api/workpaper/revenue`.
- `npm run framework-adapters` now proves Fetch, Hono, AdonisJS, Hapi, Express,
  and Fastify wrappers against the shared WorkPaper request handler, with
...
- Bundle path: `pnpm mcpb:workpaper:build -- --package-version 0.14.0`
  creates `build/mcpb/bilig-workpaper.mcpb` from the published npm package.
- Build proof: the MCPB manifest validates, packages `bilig-workpaper@0.14.0`,
...
Latest persistence-adapter distribution snapshot on May 13, 2026 after the
...
- Do not route new evaluator traffic to the older public DEV copy until it is
  ...
  checked package evidence is `@bilig/headless@0.18.25`, `100/100` comparable
  mean wins, and `100/100` mean+p95 wins.
...
The `100/100` WorkPaper mean-win claim should become small, quotable artifacts:
...
- one short post explaining the narrowest p95 row honestly
...
3/ Current benchmark artifact: 100/100 comparable mean-latency rows are faster
than the HyperFormula-style baseline.

The narrowest p95 row is public too: aggregate-overlapping-sliding-window-wide is
the visible narrowest p95 win, and UI rendering is not part of that benchmark.
...
The benchmark claim is intentionally narrow: 100/100 comparable mean-latency
wins against HyperFormula-style workloads, aggregate-overlapping-sliding-window-wide
is the visible narrowest p95 win, and UI rendering is not part of the benchmark.
...
- Do not claim blanket performance wins; keep the narrowest p95 row visible.
