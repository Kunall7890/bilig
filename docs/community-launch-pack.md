...
- External open-source directory surface: the Open-Awesome submission for Bilig
  has a metadata refresh comment with the current `@bilig/headless@0.16.26`
...
- npm surface: `@bilig/headless@0.16.26`, with `12,568` downloads in the
...
- Do not route new evaluator traffic to the older public DEV copy until it is
  ...
  checked package evidence is `@bilig/headless@0.16.26`, `72/100` comparable
  mean wins, and `70/100` mean+p95 wins.
...
The `72/100` WorkPaper mean-win claim should become small, quotable artifacts:
...
3/ Current benchmark artifact: 72/100 comparable mean-latency rows are faster
than the HyperFormula-style baseline.

The ugly part is public too: lookup-text-exact is the visible p95
...
The benchmark claim is intentionally narrow: 72/100 comparable mean-latency wins
against HyperFormula-style workloads, lookup-text-exact is the
