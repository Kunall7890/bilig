...
- Do not route new evaluator traffic to the older public DEV copy until it is
  ...
  checked package evidence is `@bilig/headless@0.41.0`, `99/100` comparable
  mean wins, and `96/100` mean+p95 wins.
...
The `99/100` WorkPaper mean-win claim should become small, quotable artifacts:
...
- one short post explaining the worst p95 row honestly
...
3/ Current benchmark artifact: 99/100 comparable mean-latency rows are faster
than the HyperFormula-style baseline.

The worst p95 row is public too: structural-append-formula-rows-large is the
...
The benchmark claim is intentionally narrow: 99/100 comparable mean-latency
wins against HyperFormula-style workloads, structural-append-formula-rows-large
...
- Do not claim blanket performance wins; keep the worst p95 row visible.
