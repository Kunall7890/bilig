...
`checkWorkbookRefData(data)` validates transported range/name/table/column/rows
refs with stable path issues before hydration or persisted proof use.
`isWorkbookRefData` is the boolean guard over the same boundary.
