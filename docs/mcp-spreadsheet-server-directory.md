...
now points reviewers at `@bilig/headless@0.30.2` and `libraries-v0.30.2`.
...
Latest checked result on May 19, 2026: Live but latest marker lags npm; `0.27.0` is latest-marked while npm latest is `0.30.2`. npm latest is `@bilig/headless@0.30.2`,
and the official Registry latest-marked entry is
`io.github.proompteng/bilig-workpaper@0.27.0` with package
`@bilig/headless@0.27.0`, so it does not yet match npm latest `@bilig/headless@0.30.2`. The API also returns historical entries, so
