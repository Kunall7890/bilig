...
now points reviewers at `@bilig/headless@0.27.0` and `libraries-v0.27.0`.
...
Latest checked result on May 19, 2026: npm latest is `@bilig/headless@0.27.0`,
and the official Registry latest-marked entry is
`io.github.proompteng/bilig-workpaper@0.27.0` with package
`@bilig/headless@0.27.0` and remote `https://bilig.proompteng.ai/mcp`.
...
printf '%s\n' \
  ...
  '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
  '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' |
  npm exec --package @bilig/headless@0.27.0 -- \
