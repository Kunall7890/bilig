...
```sh
npm exec --package @bilig/workpaper@0.83.0 -- bilig-workpaper-mcp
```
...
| ------------------------------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Official MCP Registry           | Live; latest marker matches npm latest `@bilig/workpaper@0.83.0`                       | <https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.proompteng%2Fbilig-workpaper&limit=100> |
...
now points reviewers at `@bilig/workpaper@0.83.0` and `libraries-v0.83.0`.
...
Latest checked result on May 24, 2026: Live; latest marker matches npm latest `@bilig/workpaper@0.83.0`. npm latest is `@bilig/workpaper@0.83.0`.
The official Registry latest-marked entry is
`io.github.proompteng/bilig-workpaper@0.83.0` with package
`@bilig/workpaper@0.82.0`. The API also returns historical entries, so
...
printf '%s\n' \
  ...
  '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
  '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' |
  npm exec --package @bilig/workpaper@0.83.0 -- \
