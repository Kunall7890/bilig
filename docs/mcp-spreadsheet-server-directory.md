...
Registry API now includes the current `@bilig/headless@0.19.1` package version.
...
- Local smoke proof: the same npm path installed
  `@bilig/headless@0.19.1`, started server `bilig-headless-workpaper`, exposed
...
Latest checked result on May 17, 2026: npm latest is `@bilig/headless@0.19.1`,
and the official Registry API search now includes Bilig WorkPaper entry version
`0.19.1` with `isLatest: true`. The API also returns historical entries, so
