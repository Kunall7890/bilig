...
- WorkPaper package: `@bilig/headless` `0.17.1`
...
current checked-in artifact records `78/100` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.6495453937624811`. The overall
directional p95-ratio geomean is `0.644044102975537`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `structural-insert-columns-small`, with a mean ratio of
`41.53230863787668`. The current worst p95 row is `structural-insert-columns-small`,
with a p95 ratio of `124.05108659069609`. The headless leadership scorecard
...
The `78/100` count is about mean latency: for each winning comparable workload
