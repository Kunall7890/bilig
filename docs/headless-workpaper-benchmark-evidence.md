...
- WorkPaper package: `@bilig/headless` `0.17.0`
...
current checked-in artifact records `73/100` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.6864885861101904`. The overall
directional p95-ratio geomean is `0.7157937281428121`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `structural-insert-columns-small`, with a mean ratio of
`2.2679670133805168`. The current worst p95 row is `structural-insert-columns`,
with a p95 ratio of `7.840633803451677`. The headless leadership scorecard
currently records `67/100` workloads winning both
...
The `73/100` count is about mean latency: for each winning comparable workload
