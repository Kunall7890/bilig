...
- WorkPaper package: `@bilig/headless` `0.16.26`
...
current checked-in artifact records `72/100` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.7071933537655859`. The overall
directional p95-ratio geomean is `0.6979827319598825`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `lookup-text-exact`, with a mean ratio of
`12.980318814275519`. The current worst p95 row is `lookup-text-exact`,
with a p95 ratio of `22.99194632249502`. The headless leadership scorecard
currently records `70/100` workloads winning both
...
The `72/100` count is about mean latency: for each winning comparable workload
