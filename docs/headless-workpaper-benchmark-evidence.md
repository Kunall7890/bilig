...
current checked-in artifact records `76/100` mean-latency wins:
...
The overall directional mean-ratio geomean is `0.6336586169180994`. The overall
directional p95-ratio geomean is `0.6740490944769602`. Ratios below `1.0` mean
WorkPaper is faster for that metric.

The current worst mean row is `single-formula-edit-recalc`, with a mean
ratio of `3.2176551690609276`. The current worst p95 row is
`single-formula-edit-recalc`, with a p95 ratio of `5.425557986875876`. The
...
The `76/100` count is about mean latency: for each winning comparable workload
