...
interface PrefectWorkPaperQuoteInput {
...
interface PrefectWorkPaperQuoteSummary {
...
interface PrefectWorkPaperQuoteResult {
  readonly patch: {
    readonly subtotal: number
  ...
  readonly proof: {
    ...
    readonly before: PrefectWorkPaperQuoteSummary
    readonly after: PrefectWorkPaperQuoteSummary
    readonly afterRestore: PrefectWorkPaperQuoteSummary
...
function calculateWorkPaperQuote(quoteInput: PrefectWorkPaperQuoteInput): PrefectWorkPaperQuoteResult {
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        limitations: [
          'This example proves a Prefect flow boundary with a Node WorkPaper step, not an official Prefect examples contribution.',
          'Keep Excel or another oracle in the loop for macros, pivots, external links, and exact desktop Excel behavior.',
          'The Prefect worker image or process must provide Node plus npm dependencies compatible with @bilig/workpaper.',
        ],
      }
    } finally {
      restored.dispose()
...
function readCliInput(args: readonly string[]): PrefectWorkPaperQuoteInput {
...
function readSummary(workbook: WorkPaper, summarySheet: number): PrefectWorkPaperQuoteSummary {
  return {
...
function sameSummary(left: PrefectWorkPaperQuoteSummary, right: PrefectWorkPaperQuoteSummary): boolean {
