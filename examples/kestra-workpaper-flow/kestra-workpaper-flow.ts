...
interface KestraWorkPaperQuoteInput {
...
interface KestraWorkPaperQuoteSummary {
...
interface KestraWorkPaperQuoteResult {
  readonly patch: {
    readonly subtotal: number
  ...
  readonly proof: {
    ...
    readonly before: KestraWorkPaperQuoteSummary
    readonly after: KestraWorkPaperQuoteSummary
    readonly afterRestore: KestraWorkPaperQuoteSummary
...
function calculateWorkPaperQuote(quoteInput: KestraWorkPaperQuoteInput): KestraWorkPaperQuoteResult {
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        limitations: [
          'This example proves a Kestra Node script flow boundary, not an official Kestra Blueprint.',
          'Keep Excel or another oracle in the loop for macros, pivots, external links, and exact desktop Excel behavior.',
          'The Kestra worker image must install and run a Node version compatible with @bilig/workpaper.',
        ],
      }
    } finally {
      restored.dispose()
...
function readCliInput(args: readonly string[]): KestraWorkPaperQuoteInput {
...
function readSummary(workbook: WorkPaper, summarySheet: number): KestraWorkPaperQuoteSummary {
  return {
...
function sameSummary(left: KestraWorkPaperQuoteSummary, right: KestraWorkPaperQuoteSummary): boolean {
