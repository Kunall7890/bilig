...
export interface TriggerDevWorkPaperQuoteInput {
...
export interface TriggerDevWorkPaperQuoteResult {
  readonly patch: {
  ...
  readonly proof: {
    ...
    readonly before: TriggerDevWorkPaperQuoteSummary
    readonly after: TriggerDevWorkPaperQuoteSummary
    readonly afterRestore: TriggerDevWorkPaperQuoteSummary
...
export interface TriggerDevWorkPaperQuoteSummary {
  readonly subtotal: number
...
export function calculateWorkPaperQuote(rawInput: TriggerDevWorkPaperQuoteInput): TriggerDevWorkPaperQuoteResult {
  ...
  try {
    ...
    try {
      ...
      return {
        ...
        limitations: [
          'This example proves a Trigger.dev task boundary, not an official Trigger.dev template.',
          'Keep Excel or another oracle in the loop for macros, pivots, external links, and exact desktop Excel behavior.',
          'The Trigger.dev worker must run a JavaScript runtime compatible with @bilig/workpaper.',
        ],
      }
    } finally {
      restored.dispose()
...
function normalizeInput(input: TriggerDevWorkPaperQuoteInput): Required<TriggerDevWorkPaperQuoteInput> {
...
function readSummary(workbook: WorkPaper, summarySheet: number): TriggerDevWorkPaperQuoteSummary {
  return {
...
function sameSummary(left: TriggerDevWorkPaperQuoteSummary, right: TriggerDevWorkPaperQuoteSummary): boolean {
