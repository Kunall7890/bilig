...
const smokeWorkbook = buildWorkbook()
const aiSdkTools = createAiSdkTools(createWorkPaperTools(smokeWorkbook))
...
function requireToolOutput(
  toolResults: ReadonlyArray<(typeof allToolResults)[number]>,
  toolCallId: string,
...
function requireWorkPaperReadResult(value: unknown): WorkPaperReadResult {
  if (
    isRecord(value) &&
    'range' in value &&
...
function requireWorkPaperWriteResult(value: unknown): WorkPaperWriteResult {
  if (
    isRecord(value) &&
    'editedCell' in value &&
...
function assertOutput(output: {
  ...
  if (output.apiShape !== 'AI SDK generateText -> tool -> execute') {
    throw new Error(`Unexpected API shape: ${output.apiShape}`)
  }

  if (output.modelCallCount !== 2) {
    throw new Error(`Expected two model calls, received ${output.modelCallCount}`)
  }
