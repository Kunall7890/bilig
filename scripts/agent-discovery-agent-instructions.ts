...
export function buildDocsAgentInstructions(input: DocsAgentInstructionInput): string {
  const { mcpbReleaseAssetUrl, mcpbReleaseChecksumUrl, remoteMcpEndpoint, remoteMcpServerCard, siteRoot, workpaperPackageSpec } = input
...
4. Read \`${siteRoot}/skill.txt\` when your agent supports skill manifests.
