...
export function writeXlsxImportScript(projectDir: string): void {
  writeFileSync(
    ...
    [
      ...
      '};',
      '',
      'const imported = importXlsx(exportXlsx(snapshot), "structured-financial-model.xlsx", { preferNativeSimpleImport: true });',
      ...
      'const headerFormula = importedConstants?.cells.find((cell) => cell.address === "H7")?.formula ?? "";',
      'const currentRowFormula = importedImports?.cells.find((cell) => cell.address === "D7")?.formula ?? "";',
      'const columnFormula = importedImports?.cells.find((cell) => cell.address === "F10")?.formula ?? "";',
      'const heightFormula = importedPlayerData?.cells.find((cell) => cell.address === "M4")?.formula ?? "";',
      'const structuredReferencesImported =',
      '  !headerFormula.includes("tblPFS[#Headers]") &&',
      '  [headerFormula, currentRowFormula, columnFormula, heightFormula].every((formula) => formula.length > 0 && !formula.includes("#REF!"));',
      ...
      'workbook.setCellContents({ sheet: importsId, row: 6, col: 1 }, 175);',
      'const editedTotalValue = read(importsId, 9, 5);',
      'const exportedBytes = exportXlsx(workbook.exportSnapshot());',
      'const roundTripped = importXlsx(exportedBytes, "edited-structured-financial-model.xlsx", { preferNativeSimpleImport: true });',
      'const reopened = WorkPaper.buildFromSnapshot(roundTripped.snapshot, { maxRows: 20, maxColumns: 16, useColumnIndex: true });',
      'const reopenedImportsId = reopened.getSheetId("Imports");',
      'if (reopenedImportsId === undefined) throw new Error("Round-tripped XLSX imports sheet is missing");',
      'const roundTripTotalValue = reopened.getCellValue({ sheet: reopenedImportsId, row: 9, col: 5 });',
      'const roundTripSheetNames = roundTripped.snapshot.sheets.map((sheet) => sheet.name);',
      'if (!structuredReferencesImported || currencyLabel.tag !== ValueTag.String || headerPeriod.tag !== ValueTag.Number || firstPeriod.tag !== ValueTag.Number || secondPeriod.tag !== ValueTag.Number || totalValue.tag !== ValueTag.Number || heightFeet.tag !== ValueTag.Number || editedTotalValue.tag !== ValueTag.Number || roundTripTotalValue.tag !== ValueTag.Number) {',
      '  throw new Error(`Unexpected XLSX import values: ${JSON.stringify({ structuredReferencesImported, headerFormula, currentRowFormula, columnFormula, heightFormula, currencyLabel, headerPeriod, firstPeriod, secondPeriod, totalValue, heightFeet, editedTotalValue, roundTripTotalValue })}`);',
      '}',
      ...
      '  editedTotalValue: editedTotalValue.value,',
      '  exportedBytes: exportedBytes.byteLength,',
      '  roundTripSheetNames,',
      '  roundTripTotalValue: roundTripTotalValue.value,',
