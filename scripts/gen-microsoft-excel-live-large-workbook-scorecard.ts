...
function formatJsonForRepo(serializedJson: string): string {
  const tempDir = mkdtempSync(join(tmpdir(), 'microsoft-excel-live-large-workbook-scorecard-'))
  ...
  if (formatResult.exitCode !== 0) {
    rmSync(tempDir, { recursive: true, force: true })
    throw new Error(
      `Unable to format generated Microsoft Excel live large-workbook scorecard: ${new TextDecoder().decode(formatResult.stderr).trim()}`,
    )
  }
