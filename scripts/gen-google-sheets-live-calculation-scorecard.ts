...
function formatJsonForRepo(serializedJson: string): string {
  const tempDir = mkdtempSync(join(tmpdir(), 'google-sheets-live-calculation-scorecard-'))
  ...
  if (formatResult.exitCode !== 0) {
    ...
    throw new Error(
      `Unable to format generated Google Sheets live calculation scorecard: ${new TextDecoder().decode(formatResult.stderr).trim()}`,
    )
  }
