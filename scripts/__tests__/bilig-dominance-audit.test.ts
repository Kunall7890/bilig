  ...
  it('keeps UI proof gaps visible while surfacing local browser-capture guards', () => {
    ...
    expect(uiItem).toMatchObject({
      ...
      liveBlockers: expect.arrayContaining([
        ...
        `same-corpus UI proof has 0/${String(requiredUiResponsivenessSameCorpusWorkloads.length)} Bilig authoritative render proof cases`,
    ...
    uiResponsivenessLiveBrowserScorecard: {
      ...
      sameCorpusProof: {
        ...
        runManifest: {
          ...
          invalidReasons: [
            'mutation target proof covers 0/18 required per-sample product proofs',
            'committed target proof timing covers 0/18 required per-sample product timings',
          ],
        },
      },
    },
...
function sameCorpusScenarioProof(biligMs: number, googleMs: number) {
  ...
  return {
    ...
    meanRatio: googleMs / biligMs,
    p95Ratio: googleMs / biligMs,
    microsoftExcelWebMeanRatio: microsoftExcelWebMs / biligMs,
    microsoftExcelWebP95Ratio: microsoftExcelWebMs / biligMs,
    screenshotProof: {
      captured: true,
