...
describe('repository dependency policy', () => {
  ...
  it('keeps workbook compatibility file reports on file-backed native scans', () => {
    ...
    const externalWorkbookGuardIndex = reportSource.indexOf('assertXlsxExternalWorkbookByteInputWithinLimit(byteLength, workbook.path)')
    const externalWorkbookReadIndex = reportSource.indexOf('bytes: readFileSync(workbook.path)')
