...
const generatedReportPath = path.join(repoRoot, 'packages/formula/src/generated/formula-inventory.ts')
...
const protocolBuiltinsByName = new Map(BUILTINS.map((builtin) => [builtin.name.toUpperCase(), builtin]))
...
function getVariableInitializer(sourceFile: ts.SourceFile, variableName: string): ts.Expression | undefined {
...
async function readSourceDerivedRuntimeData(): Promise<SourceDerivedRuntimeData> {
  ...
  const placeholderNames = new Set([
    ...
    ...allAdditionalExcelScalarBuiltinNames.filter((name) => !implementedScalarPlaceholderBuiltinNames.has(name)),
  ])
...
async function inferRuntimeStatus(name: string): Promise<FormulaRuntimeStatus> {
  ...
  return runtimeLookupNames(name).some((entry) => runtimeData.implementedBuiltinNames.has(entry)) ? 'implemented' : 'missing'
}
...
async function inferJsStatus(name: string): Promise<FormulaRuntimeJsStatus> {
  ...
  return runtimeLookupNames(name).some((entry) => runtimeData.jsSpecialBuiltinNames.has(entry)) ? 'special-js-only' : 'implemented'
}

async function inferWasmStatus(name: string): Promise<FormulaRuntimeWasmStatus> {
  const runtimeData = await getSourceDerivedRuntimeData()
  return runtimeLookupNames(name).some((entry) => runtimeData.wasmProductionBuiltinNames.has(entry)) ? 'production' : 'not-started'
}
...
async function renderInventoryReport(source: FormulaInventorySource): Promise<string> {
  ...
  const summary = {
    ...
    officeListedRegisteredInCodebase: entries.filter((entry) => entry.inOfficeList && entry.registeredInCodebase).length,
    overlap: entries.filter((entry) => summarizeOdfMembership(entry.odfStatus) && entry.inOfficeList).length,
    odfOnly: entries.filter((entry) => summarizeOdfMembership(entry.odfStatus) && !entry.inOfficeList).length,
    officeOnly: entries.filter((entry) => !summarizeOdfMembership(entry.odfStatus) && entry.inOfficeList).length,
  ...
  const summaryLines = Object.entries(summary).map(([key, value]) => `  ${escapeTsPropertyName(key)}: ${value},`)
...
async function renderInventoryDoc(source: FormulaInventorySource): Promise<string> {
  const runtimeStatuses = await Promise.all(source.entries.map((entry) => inferRuntimeStatus(normalizeFormulaName(entry.name))))
  ...
  const odfMandatory = source.entries.filter((entry) => summarizeOdfMembership(entry.odfStatus)).length
  const officeCount = source.entries.filter((entry) => entry.inOfficeList).length
  const overlap = source.entries.filter((entry) => summarizeOdfMembership(entry.odfStatus) && entry.inOfficeList).length
  const odfOnly = source.entries.filter((entry) => summarizeOdfMembership(entry.odfStatus) && !entry.inOfficeList).length
  const officeOnly = source.entries.filter((entry) => !summarizeOdfMembership(entry.odfStatus) && entry.inOfficeList).length
...
async function readSource(): Promise<FormulaInventorySource> {
  ...
  for (const builtin of BUILTINS) {
    ...
    const hasAlias = source.entries.some((entry) => runtimeLookupNames(entry.name).includes(builtinName))
...
async function writeGeneratedFiles(checkMode: boolean): Promise<void> {
  ...
  const staleOutputs = outputs.filter((output, index) => existingOutputs[index] !== output.contents)
  if (!checkMode) {
    await Promise.all(staleOutputs.map((output) => writeFile(output.path, output.contents, 'utf8')))
  } else {
