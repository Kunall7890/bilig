...
export function requireHomepageDiscovery(index: string, siteCss: string): void {
  requireAllIncludes(
    ...
    [
      ...
      '<dd>46 / 46 rows ahead</dd>',
      'Only the comparable WorkPaper vs HyperFormula rows in the repo.',
      'packages/benchmarks/baselines/<wbr />workpaper-vs-hyperformula.json',
      'If the benchmark moves, review shows the JSON diff.',
      '<code>lookup-approximate-duplicates</code> p95 1.043x',
      'That case is slower. If your workbook looks like it, test first.',
