...
export function requireHomepageDiscovery(index: string, siteCss: string, productCss: string, docsRoot: string): void {
  ...
  requireAllIncludes(
    ...
    [
      ...
      'content="WorkPaper runtime, spreadsheet formula readback, headless workbook runtime, spreadsheet formulas Node.js, backend spreadsheet formulas, agent workbook automation, agent MCP tools, MCP spreadsheet tools, JSON WorkPaper persistence, @bilig/headless, @bilig/workpaper, XLSX formula recalculation Node.js"',
      ...
      'Spreadsheet formula readback before backend code or an agent uses the result',
      'Agent MCP tool contracts',
      'tools only when saved files are still the contract.',
