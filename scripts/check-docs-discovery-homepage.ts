...
export function requireHomepageDiscovery(index: string, siteCss: string): void {
  requireAllIncludes(
    ...
    [
      ...
      '<title>bilig - Spreadsheet Models for TypeScript</title>',
      ...
      'no browser required',
      'Use it when a spreadsheet is the model, not the interface.',
      'Pricing tables, budget checks, imports, payouts, and agent tools often start as rows and formulas.',
      'Write the input.',
      'Run the smoke test before you trust the docs.',
      'checks that the recalculated total survived the round trip.',
      ...
      'Try the package, drop it behind a service route, expose it as an agent tool',
      'Take a starter issue that improves the examples.',
      'Reference by job, not by package name.',
      'Open the install path, service recipe, agent adapter, or comparison page that matches what you are building.',
      '<h3>Run</h3>',
      '<h3>Build</h3>',
      '<h3>Agents</h3>',
      '<h3>Decide</h3>',
