...
export function requireHomepageDiscovery(index: string, siteCss: string, productCss: string, docsRoot: string): void {
  ...
  requireAllIncludes(
    ...
    [
      ...
      "const publicDocsHostnames = new Set(['proompteng.github.io', 'bilig.proompteng.ai'])",
      'const isHostedSourcePreview',
      'const isSourcePreview',
      '!publicDocsHostnames.has(window.location.hostname)',
      'document.querySelectorAll',
      'markdownHrefForLocalSource',
      'rewriteLocalSourceLinks',
      'if (isSourcePreview) {',
      "document.addEventListener('click', rewriteClickedLocalSourceLink)",
  ...
  requireNoIncludes(
    ...
    [
      ...
      'const canProbeLocalHtml',
      'const localHtmlLinks',
      'const probeAndRewriteMissingLocalHtml',
      "document.addEventListener('click', guardClickedLocalHtmlLink)",
      "fetch(href, { method: 'HEAD', cache: 'no-store' })",
      'window.location.href = response.status === 404 ? rewrittenHref : href',
