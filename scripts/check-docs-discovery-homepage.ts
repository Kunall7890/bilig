...
export function requireHomepageDiscovery(index: string, siteCss: string, productCss: string, docsRoot: string): void {
  ...
  requireAllIncludes(
    ...
    [
      ...
      '<h1 id="hero-title" class="hero-title">Fix stale formula values before your Node job returns.</h1>',
      'Keep SheetJS, ExcelJS, or xlsx-populate for file work.',
      'Add Bilig at the calculation boundary when the cached formula value is',
