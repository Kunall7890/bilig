...
function formatJsonForRepo(serializedJson: string): string {
  const tempDir = mkdtempSync(join(tmpdir(), 'microsoft-excel-live-recalculation-scorecard-'))
  ...
  if (formatResult.exitCode !== 0) {
    rmSync(tempDir, { recursive: true, force: true })
    throw new Error(
      `Unable to format generated Microsoft Excel live recalculation scorecard: ${new TextDecoder().decode(formatResult.stderr).trim()}`,
    )
  }
