...
function formatJsonForRepo(serializedJson: string): string {
  const tempDir = mkdtempSync(join(tmpdir(), 'microsoft-excel-live-calculation-scorecard-'))
  ...
  if (formatResult.exitCode !== 0) {
    rmSync(tempDir, { recursive: true, force: true })
    throw new Error(
      `Unable to format generated Microsoft Excel live calculation scorecard: ${new TextDecoder().decode(formatResult.stderr).trim()}`,
    )
  }
