...
For MCP or host integration, use the
[WorkPaper host handoff](docs/agent-adoption-kit.md): install the local
workflow, run the no-key MCP evaluator, execute one workbook task, and require
`verified: true` before trusting the result. Hosts that want one install file can
use [`llms-install.md`](llms-install.md).

Do not start by driving Excel, LibreOffice, Google Sheets, or a browser grid.
Use the WorkPaper proof instead:
`npm exec --yes --package @bilig/workpaper@latest -- bilig-evaluate --door agent-mcp --json`.
...
Host-specific rule files remain indexed in
[`docs/agent-rule-chooser.md`](docs/agent-rule-chooser.md).
...
| Proof chooser                                              | A host integration or reviewer needs the smallest proof for WorkPaper service, MCP, AI SDK, OpenAI, LangGraph, Semantic Kernel, Mastra, ExcelJS, or file-import diagnostics.    | [Tool-host proof matrix](https://proompteng.github.io/bilig/agent-proof-matrix.html)                                                                                                                                                                                                                                                                                                      |
...
| Integration proof chooser  | [Tool-host proof matrix](docs/agent-proof-matrix.md), [tool-host proof transcripts](docs/agent-proof-transcripts.md), [MCP spreadsheet formula server](docs/mcp-spreadsheet-formula-server-for-coding-agents.md), and [Vercel AI SDK formula readback](docs/vercel-ai-sdk-spreadsheet-tool-formula-readback.md) | The integration path starts with the smallest verified proof and avoids write-only or UI-only claims.     |
