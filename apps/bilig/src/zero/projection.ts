...
function buildWorkbookHeaderRow(documentId: string, snapshot: WorkbookSnapshot, options: WorkbookProjectionOptions): WorkbookSourceRow {
...
function buildCalculationSettingsRow(documentId: string, snapshot: WorkbookSnapshot): CalculationSettingsSourceRow {
  const calcSettings = snapshot.workbook.metadata?.calculationSettings
  const recalcEpoch = snapshot.workbook.metadata?.volatileContext?.recalcEpoch ?? 0
  return {
...
export function buildCalculationSettingsRowFromEngine(documentId: string, engine: SpreadsheetEngine): CalculationSettingsSourceRow {
  const calcSettings = engine.getCalculationSettings()
  const recalcEpoch = engine.getVolatileContext().recalcEpoch ?? 0
  return {
    workbookId: documentId,
...
export function buildSingleCellSourceRowFromEngine(
  ...
  return buildCellSourceRowFromEngineCell(documentId, sheetName, address, row, col, engine.getCell(sheetName, address), options)
}
...
function buildWorkbookStyleRows(documentId: string, snapshot: WorkbookSnapshot, options: WorkbookProjectionOptions): StyleSourceRow[] {
...
export function buildWorkbookSourceProjectionFromEngine(
  ...
  for (const sheet of [...engine.workbook.sheetsByName.values()].toSorted((left, right) => left.order - right.order)) {
    ...
    rowMetadata.push(...buildSheetRowMetadataRowsFromEngine(documentId, engine, sheet.name, options))
    columnMetadata.push(...buildSheetColumnMetadataRowsFromEngine(documentId, engine, sheet.name, options))
  }
...
export const sourceProjectionKeys = {
  ...
  axisMetadata: (row: AxisMetadataSourceRow) => JSON.stringify([row.workbookId, row.sheetName, row.startIndex]),
