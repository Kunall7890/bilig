...
describe('publication-store', () => {
  ...
  it('adds only the missing replicated tables when the publication already exists', async () => {
    ...
    expect(query.text).toContain(`public."workbook_chat_item"`)
    expect(query.text).toContain(`public."workbook_chat_tool_call"`)
    expect(query.text).toContain(`public."workbook_review_queue_item"`)
  ...
  it('rejects invalid publication names from the environment', () => {
    expect(() => resolveZeroPublicationName({ BILIG_ZERO_PUBLICATION: 'bad-name!' })).toThrow('Invalid Zero publication name: bad-name!')
  })
})
