...
export function normalizeWorkbookSheetIdAssignments(rows: readonly WorkbookSheetIdRow[]): WorkbookSheetIdAssignment[] {
  const orderedRows = rows.toSorted(
    (left, right) =>
      left.workbookId.localeCompare(right.workbookId) || left.sortOrder - right.sortOrder || left.name.localeCompare(right.name),
  )
  ...
  while (index < orderedRows.length) {
    ...
    workbookRows.forEach((row) => {
      if (!isValidSheetId(row.sheetId) || counts.get(row.sheetId) !== 1 || usedSheetIds.has(row.sheetId)) {
  ...
  return assignments.toSorted((left, right) => left.workbookId.localeCompare(right.workbookId) || left.nextSheetId - right.nextSheetId)
}

async function loadExistingTables(db: Queryable, tableNames: readonly string[]): Promise<ReadonlySet<string>> {
  ...
  return new Set(result.rows.flatMap((row) => (typeof row.tableName === 'string' && row.tableName.length > 0 ? [row.tableName] : [])))
}
