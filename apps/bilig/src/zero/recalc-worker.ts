  ...
  private async processNextJob(): Promise<boolean> {
    ...
    try {
      await this.runtimeManager.runExclusive(lease.workbookId, async () => {
        ...
        const nextReplicaSnapshot = shouldCheckpoint ? runtime.engine.exportReplicaSnapshot() : null
        const completed = await markRecalcJobCompleted(this.db, lease, cellEvalRows, nextSnapshot, nextReplicaSnapshot, true)
        if (!completed) {
