  ...
  private applyProjectionCommit(session: RuntimeSession, commit: WorkbookProjectionCommit): void {
    switch (commit.kind) {
      ...
      case 'focused-cell':
        ...
        session.projection.cells = withUpsertedProjectionCell(session.projection.cells, commit.sheetName, commit.address, commit.cell)
        return
      case 'cell-range':
        session.projection.workbook = commit.workbook
        session.projection.calculationSettings = commit.calculationSettings
        ...
        session.projection.cells = withReplacedProjectionCellsInRange(session.projection.cells, commit.range, commit.cells)
        return
...
function withUpsertedProjectionCell(
  ...
  const index = cells.findIndex((entry) => entry.sheetName === sheetName && entry.address === address)
  if (!nextRow) {
    if (index >= 0) {
