...
export interface LocalSnapshotStoreContext<SessionState extends LocalSnapshotSessionState = LocalSnapshotSessionState> {
...
export function invalidateLocalSnapshotCache<SessionState extends LocalSnapshotSessionState>(session: SessionState): void {
...
export function getLocalCachedSnapshot<SessionState extends LocalSnapshotSessionState>(session: SessionState): WorkbookSnapshot {
...
export function acceptLocalSnapshotChunk<SessionState extends LocalSnapshotSessionState>(
  ...
  storeLocalCachedSnapshot(session, snapshot)
  publication.frames.forEach((snapshotFrame) => context.broadcast(session.documentId, snapshotFrame))
  context.broadcast(session.documentId, createCursorWatermarkFrame(session.documentId, assembled.cursor, assembled.cursor))
}
