...
export function isWorkbookAgentRolloutAllowed(
  ...
  return featureFlags.allowlistedUserIds.includes(input.userId) || featureFlags.allowlistedDocumentIds.includes(input.documentId)
}

export type WorkbookAgentWorkflowFamily = 'report' | 'formula' | 'formatting' | 'import' | 'rollup' | 'structural'
