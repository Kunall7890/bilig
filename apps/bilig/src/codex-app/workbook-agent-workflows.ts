...
import { searchWorkbook, summarizeWorkbookStructure, traceWorkbookDependencies } from './workbook-agent-comprehension.js'
import { executeFormattingWorkflow, getFormattingWorkflowTemplateMetadata } from './workbook-agent-formatting-workflows.js'
import { executeFormulaWorkflow, getFormulaWorkflowTemplateMetadata } from './workbook-agent-formula-workflows.js'
import { executeImportWorkflow, getImportWorkflowTemplateMetadata } from './workbook-agent-import-workflows.js'
import { executeRollupWorkflow, getRollupWorkflowTemplateMetadata } from './workbook-agent-rollup-workflows.js'
import {
...
import {
  serializeWorkflowCellValue,
  summarizeCellExplanationMarkdown,
  summarizeCurrentSheetMarkdown,
  summarizeDependencyTraceMarkdown,
  summarizeRecentChangesMarkdown,
  summarizeSearchResultsMarkdown,
  summarizeWorkbookMarkdown,
} from './workbook-agent-workflow-markdown.js'
...
type FormulaWorkflowTemplate = 'findFormulaIssues' | 'highlightFormulaIssues' | 'repairFormulaIssues'
...
function isFormulaWorkflowTemplate(workflowTemplate: WorkbookAgentWorkflowTemplate): workflowTemplate is FormulaWorkflowTemplate {
  return (
    workflowTemplate === 'findFormulaIssues' || workflowTemplate === 'highlightFormulaIssues' || workflowTemplate === 'repairFormulaIssues'
  )
}

function isFormattingWorkflowTemplate(workflowTemplate: WorkbookAgentWorkflowTemplate): workflowTemplate is FormattingWorkflowTemplate {
  return workflowTemplate === 'highlightCurrentSheetOutliers' || workflowTemplate === 'styleCurrentSheetHeaders'
}

function isImportWorkflowTemplate(workflowTemplate: WorkbookAgentWorkflowTemplate): workflowTemplate is ImportWorkflowTemplate {
  return (
...
function isRollupWorkflowTemplate(workflowTemplate: WorkbookAgentWorkflowTemplate): workflowTemplate is RollupWorkflowTemplate {
  return workflowTemplate === 'createCurrentSheetRollup' || workflowTemplate === 'createCurrentSheetReviewTab'
}
...
function getWorkflowTemplateMetadata(
  ...
  if (
    ...
    throw new Error(`Unexpected workflow template metadata resolution fell through: ${workflowTemplate}`)
  }
      ...
      return {
        ...
        stepPlans: [
          ...
          },
          {
            ...
            pendingSummary: 'Waiting to load the selected cell value, formula state, and workbook links.',
          },
          {
...
export function createRunningWorkflowSteps(
  ...
  return getWorkflowTemplateMetadata(workflowTemplate, workflowInput).stepPlans.map((step, index) => ({
    stepId: step.stepId,
    label: step.label,
...
export function failWorkflowSteps(
  ...
  return getWorkflowTemplateMetadata(workflowTemplate, workflowInput).stepPlans.map((step, index) => {
    ...
    return {
      ...
      updatedAtUnixMs: current?.status === 'completed' ? current.updatedAtUnixMs : (current?.updatedAtUnixMs ?? now),
    }
  })
}

export function cancelWorkflowSteps(runningSteps: readonly WorkbookAgentWorkflowStep[], now: number): WorkbookAgentWorkflowStep[] {
  return runningSteps.map((step) => {
    ...
    return {
      ...
      status: 'cancelled',
      summary:
        step.status === 'running' ? 'Workflow cancelled before this step completed.' : 'Workflow cancelled before this step started.',
      updatedAtUnixMs: now,
    }
  })
}
...
export async function executeWorkbookAgentWorkflow(input: {
  ...
  if (isStructuralWorkflowTemplate(input.workflowTemplate)) {
    throw new Error(`Structural workflow execution unexpectedly fell through: ${input.workflowTemplate}`)
  }
  ...
  switch (workflowTemplate as string) {
    case 'summarizeWorkbook': {
      throwIfWorkflowCancelled(input.signal)
      const structure = await input.zeroSyncService.inspectWorkbook(input.documentId, (runtime) => summarizeWorkbookStructure(runtime))
      throwIfWorkflowCancelled(input.signal)
      return {
        title: 'Summarize Workbook',
    ...
    case 'traceSelectionDependencies': {
      ...
      throwIfWorkflowCancelled(input.signal)
      const dependencyTrace = await input.zeroSyncService.inspectWorkbook(input.documentId, (runtime) =>
      ...
      throwIfWorkflowCancelled(input.signal)
      const citedNodes = [dependencyTrace.root, ...dependencyTrace.layers.flatMap((layer) => [...layer.precedents, ...layer.dependents])]
      return {
        title: 'Trace Selection Dependencies',
        summary:
          dependencyTrace.summary.precedentCount === 0 && dependencyTrace.summary.dependentCount === 0
