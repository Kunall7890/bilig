...
export const workbookAgentObjectToolSpecs = [
  ...
  },
  {
    ...
    description: 'List workbook charts with source ranges, chart types, anchor positions, and footprint size.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {},
  ...
  },
  {
    ...
    description: 'Create a workbook named range or named reference from either an explicit value payload or a semantic selector.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['name'],
  ...
  },
  {
    ...
    description: 'Update a workbook named range or named reference from either an explicit value payload or a semantic selector.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['name'],
...
function defaultColumnNames(input: {
  ...
  for (let col = start.col; col <= end.col; col += 1) {
    if (input.headerRow) {
      const cell = input.runtime.engine.getCell(input.range.sheetName, formatAddress(start.row, col))
      const candidate =
...
async function resolvePivotDeleteTarget(
  ...
  const matches = runtime.engine.getPivotTables().filter((pivot) => pivot.name.trim().toUpperCase() === args.name.trim().toUpperCase())
  if (matches.length === 0) {
...
export async function handleWorkbookAgentObjectToolCall(
  ...
  switch (normalizedTool) {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.deletePivotTable: {
      ...
      const target = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => resolvePivotDeleteTarget(runtime, args))
      return await stageCommandResult(context, {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.updateChart: {
      ...
      const chart = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => {
        ...
        return {
          ...
          ...(args.firstRowAsHeaders !== undefined ? { firstRowAsHeaders: args.firstRowAsHeaders } : {}),
          ...(args.firstColumnAsLabels !== undefined ? { firstColumnAsLabels: args.firstColumnAsLabels } : {}),
