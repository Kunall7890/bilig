...
function valueToText(value: { readonly tag: ValueTag; readonly value?: string | number | boolean; readonly code?: number }): string {
  switch (value.tag) {
    case ValueTag.Empty:
      return ''
...
function describeTraceNode(runtime: WorkbookRuntime, sheetName: string, address: string): WorkbookDependencyTraceNode {
...
function getFormulaIssueReport(runtime: WorkbookRuntime): WorkbookFormulaIssueReport {
  ...
  if (cached && cached.headRevision === runtime.headRevision && cached.calculatedRevision === runtime.calculatedRevision) {
  ...
  for (const sheet of snapshot.sheets) {
    ...
    for (const cell of sheet.cells) {
      ...
      entries.push({
        ...
        snippet,
        searchText: [sheet.name, cell.address, `${sheet.name}!${cell.address}`, formula ?? '', inputText ?? '', valueText]
          .join('\n')
          .toLowerCase(),
      })
    }
...
function scoreSearchEntry(
  ...
  const exactAddressMatch =
    qualifiedAddress !== null && (qualifiedAddress.toLowerCase() === queryLower || entry.address?.toLowerCase() === queryLower)
