...
import { buildCellNumberFormatCode, type CellNumberFormatPreset, type WorkbookSnapshot } from '@bilig/protocol'
import type { ZeroSyncService } from '../zero/service.js'
import {
  summarizeFormulaFillMarkdown,
  summarizeHeaderNormalizationMarkdown,
  summarizeNumberFormatNormalizationMarkdown,
  summarizeWhitespaceNormalizationMarkdown,
...
function inferNumberFormatPreset(headerLabel: string, numericValues: readonly number[]): CellNumberFormatPreset | null {
  ...
  if (/(?:revenue|sales|amount|price|cost|expense|profit|budget|income|total|subtotal|usd|eur|gbp|arr|mrr|gmv)/u.test(headerKey)) {
    return {
      kind: 'currency',
      currency: 'USD',
      decimals: Math.max(2, fractionDigits),
      ...
      return {
        ...
        stepPlans: [
          ...
          },
          {
            ...
            runningSummary: 'Staging the semantic change set that normalizes the current sheet header row.',
            pendingSummary: 'Waiting to stage the semantic change set that normalizes the current sheet header row.',
          },
          {
      ...
      return {
        ...
        stepPlans: [
          ...
          },
          {
            ...
            runningSummary: 'Preparing semantic number-format commands for the current sheet data columns.',
            pendingSummary: 'Waiting to prepare semantic number-format commands for the current sheet data columns.',
          },
          {
      ...
      return {
        ...
        stepPlans: [
          ...
          },
          {
            ...
            runningSummary: 'Staging the semantic change set that trims and collapses whitespace across the current sheet.',
            pendingSummary: 'Waiting to stage the semantic change set that trims and collapses whitespace across the current sheet.',
          },
          {
      ...
      return {
        ...
        stepPlans: [
          ...
          },
          {
            ...
            pendingSummary: 'Waiting to prepare semantic fill-down commands for the detected formula gaps.',
          },
          {
  ...
  return await input.zeroSyncService.inspectWorkbook(input.documentId, (runtime) => {
    ...
    if (input.workflowTemplate === 'normalizeCurrentSheetNumberFormats') {
      ...
      for (let col = minCol; col <= maxCol; col += 1) {
        ...
        const headerLabel = headerCell && typeof headerCell.value === 'string' ? normalizeHeaderLabel(headerCell.value) : ''
      ...
      const dataStartAddress = dataStartRow <= maxRow ? formatAddress(dataStartRow, minCol) : headerStartAddress
      const dataEndAddress = dataStartRow <= maxRow ? formatAddress(maxRow, maxCol) : headerEndAddress
      return {
        title: 'Normalize Current Sheet Number Formats',
        summary:
    ...
    if (input.workflowTemplate === 'normalizeCurrentSheetWhitespace') {
      ...
      return {
        ...
        citations: [createHeaderCitation(sheetName, rangeStartAddress, rangeEndAddress, changes.length > 0 ? 'target' : 'source')],
        steps: [
          {
            stepId: 'inspect-text-cells',
            label: 'Inspect text cells',
    ...
    if (input.workflowTemplate === 'fillCurrentSheetFormulasDown') {
      ...
      return {
        ...
        citations: recommendations.map((recommendation) =>
          createHeaderCitation(sheetName, recommendation.targetStartAddress, recommendation.targetEndAddress, 'target'),
        ),
        steps: [
          {
            stepId: 'inspect-formula-columns',
