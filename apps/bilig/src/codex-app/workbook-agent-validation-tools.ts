...
export const workbookAgentValidationToolSpecs = [
  {
    ...
    description: 'List workbook data validation rules. Optionally filter the result to a specific explicit range or semantic selector.',
    inputSchema: {
      type: 'object',
...
function normalizeValidationRule(rule: z.infer<typeof validationRuleSchema>): WorkbookDataValidationRuleSnapshot {
  switch (rule.kind) {
    case 'list': {
...
export async function handleWorkbookAgentValidationToolCall(
  ...
  switch (normalizedTool) {
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.updateDataValidation: {
      ...
      const validation = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => {
        const resolved = resolveRangeOrSelectorRequest({
          runtime,
          args: {
            ...(args.range ? { range: args.range } : {}),
