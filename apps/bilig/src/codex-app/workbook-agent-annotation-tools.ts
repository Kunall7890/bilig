...
export const workbookAgentAnnotationToolSpecs = [
  {
    ...
    description: 'List workbook comment threads and notes. Optionally filter to an explicit range or semantic selector.',
    inputSchema: {
      type: 'object',
...
function assertSingleCellRange(range: { readonly sheetName: string; readonly startAddress: string; readonly endAddress: string }): {
  sheetName: string
  address: string
} {
  ...
  switch (normalizedTool) {
    case WORKBOOK_AGENT_TOOL_NAMES.getComments: {
      ...
      const payload = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => {
        ...
        return {
          documentId: context.documentId,
          commentThreadCount: annotations.commentThreads.filter((thread) => annotationsIntersect(resolved.range, thread)).length,
          noteCount: annotations.notes.filter((note) => annotationsIntersect(resolved.range, note)).length,
          commentThreads: annotations.commentThreads.filter((thread) => annotationsIntersect(resolved.range, thread)),
    ...
    case WORKBOOK_AGENT_TOOL_NAMES.addComment: {
      ...
      const thread = await context.zeroSyncService.inspectWorkbook(context.documentId, (runtime) => {
        const resolved = resolveRangeOrSelectorRequest({
          runtime,
          args: {
            ...(args.range ? { range: args.range } : {}),
