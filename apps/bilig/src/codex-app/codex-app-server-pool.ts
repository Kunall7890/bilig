...
export function isCodexAppServerPoolBackpressureError(value: unknown): value is CodexAppServerPoolBackpressureError {
...
export class CodexAppServerClientPool implements CodexAppServerTransport {
  ...
  async threadResume(
    ...
    const slot = (await this.getSlotForThread(input.threadId)) ?? (await this.getSlotForNewThread())
  ...
  private async getSlotForNewThread(): Promise<CodexAppServerPoolSlot> {
    ...
    const leastLoadedSlot = [...this.slots.values()].toSorted((left, right) => this.getSlotLoad(left) - this.getSlotLoad(right))[0]
    if (!leastLoadedSlot) {
  ...
  private async createSlot(): Promise<CodexAppServerPoolSlot> {
    ...
    const fallback = [...this.slots.values()].toSorted((left, right) => this.getSlotLoad(left) - this.getSlotLoad(right))[0]
    if (!fallback) {
  ...
  private async acquireTurnPermit(slot: CodexAppServerPoolSlot, threadId: string): Promise<() => void> {
