...
export function useWorkbookEditorConflict(input: {
  ...
  useEffect(() => {
    ...
    if (selection.sheetName !== targetSelection.sheetName || selection.address !== targetSelection.address) {
      return
    }
    ...
    setEditorConflict((current) => {
      const nextPhase =
        current?.sheetName === targetSelection.sheetName && current?.address === targetSelection.address && current.phase === 'compare'
          ? 'compare'
          : 'badge'
      if (
  ...
  const reviewEditorConflict = useCallback(() => {
    const targetSelection = editorTargetRef.current
    setEditorConflict((current) => {
      if (current?.sheetName !== targetSelection.sheetName || current?.address !== targetSelection.address) {
        return current
      }
      return {
        ...current,
