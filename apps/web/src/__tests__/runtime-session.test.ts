        ...
        readViewportProjection(sheetName, viewport) {
          ...
          for (let row = viewport.rowStart; row <= viewport.rowEnd; row += 1) {
            for (let col = viewport.colStart; col <= viewport.colEnd; col += 1) {
              ...
              if (snapshot.value.tag === ValueTag.Empty && snapshot.input == null && !snapshot.formula) {
                continue
              }
...
function expectPendingMutationSummaries(value: unknown): Array<{ id: string; status: 'local' | 'submitted' | 'rebased' | 'failed' }> {
  ...
  return value.flatMap((entry) => {
    if (
      ...
      (entry['status'] !== 'local' && entry['status'] !== 'submitted' && entry['status'] !== 'rebased' && entry['status'] !== 'failed')
    ) {
...
describe('createWorkerRuntimeSessionController', () => {
  ...
  it('applies local mutations through worker viewport patches', async () => {
    ...
    const unsubscribe = controller.subscribeViewport('Sheet1', { rowStart: 0, rowEnd: 0, colStart: 0, colEnd: 0 }, () => {})
  ...
  it('preserves case-insensitive string comparison results for pending local formulas', async () => {
    ...
    const unsubscribe = controller.subscribeViewport('Sheet1', { rowStart: 0, rowEnd: 1, colStart: 0, colEnd: 0 }, () => {})
