...
describe('workbook shortcut dialog', () => {
  ...
  it('formats shortcut labels per platform', () => {
    ...
    expect(getWorkbookShortcutLabel('paste-values-only', 'MacIntel')).toBe('⇧⌘V')
    expect(getWorkbookShortcutLabel('paste-values-only', 'Win32')).toBe('Ctrl+Shift+V')
    ...
    expect(getWorkbookShortcutLabel('context-menu', 'MacIntel')).toBe('⇧⌘\\')
    expect(getWorkbookShortcutLabel('context-menu', 'Win32')).toBe('Ctrl+Shift+\\')
  ...
  it('splits shortcuts into readable keycap parts', () => {
    ...
    expect(getWorkbookShortcutParts('paste-values-only', 'MacIntel')).toEqual(['⇧', '⌘', 'V'])
    expect(getWorkbookShortcutParts('clear-formatting', 'MacIntel')).toEqual(['⌘', '\\'])
    expect(getWorkbookShortcutParts('context-menu', 'MacIntel')).toEqual(['⇧', '⌘', '\\'])
    expect(getWorkbookShortcutParts('next-sheet', 'MacIntel')).toEqual(['⌥', '↓'])
    expect(getWorkbookShortcutParts('undo', 'Win32')).toEqual(['Ctrl', 'Z'])
    expect(getWorkbookShortcutParts('paste-values-only', 'Win32')).toEqual(['Ctrl', 'Shift', 'V'])
    expect(getWorkbookShortcutParts('context-menu', 'Win32')).toEqual(['Ctrl', 'Shift', '\\'])
  })
})
