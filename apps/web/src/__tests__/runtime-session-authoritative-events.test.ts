...
describe('worker runtime session authoritative event loading', () => {
  ...
  it('hydrates an imported snapshot without polling events when event sync is disabled', async () => {
    channel = new MessageChannel()
    let state = runtimeState({
      authoritativeRevision: 0,
      pendingMutationSummary: {
    ...
    const installAuthoritativeSnapshot = vi.fn((input: { authoritativeRevision: number }) => {
      state = runtimeState({
        workbookName: 'imported workbook',
    ...
    host = createWorkerEngineHost(
      {
        ...
        getAuthoritativeRevision() {
          return state.authoritativeRevision
        },
        getRuntimeState() {
          return state
        },
    ...
    const fetchImpl = vi.fn(async (url: RequestInfo | URL) => {
      const requestUrl = typeof url === 'string' ? url : url instanceof URL ? url.href : url.url
      if (requestUrl.endsWith('/snapshot/latest')) {
        return new Response(JSON.stringify(importedSnapshot), {
          status: 200,
          headers: {
            'content-type': 'application/vnd.bilig.workbook+json',
            'x-bilig-snapshot-cursor': '1',
          },
        })
      }
      throw new Error(`Unexpected authoritative event fetch: ${requestUrl}`)
    })
    const onError = vi.fn()
    ...
    expect(installAuthoritativeSnapshot).toHaveBeenCalledWith({
      snapshot: importedSnapshot,
      authoritativeRevision: 1,
      mode: 'bootstrap',
    })
    expect(controller.runtimeState.sheetNames).toEqual(['Imported'])
    expect(controller.selection).toEqual({ sheetName: 'Imported', address: 'A1' })
    await new Promise((resolve) => setTimeout(resolve, 20))
    expect(onError).not.toHaveBeenCalled()
    expect(fetchImpl.mock.calls.map(([url]) => (typeof url === 'string' ? url : url instanceof URL ? url.href : url.url))).toEqual([
      '/v2/documents/xlsx%3Aimported/snapshot/latest',
    ])
  })
